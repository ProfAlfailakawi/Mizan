...
async function startServer() {
  ...
  app.get('/api/science/quran/kfgqpc/catalog',sensitiveIdentityRateLimit,requireGovernanceRoles(['scientific_admin','super_admin','org_admin','comp_admin','auditor']),(req,res)=>{const catalog=serverQuranSources?serverQuranSources.catalog():KFGQPC_OFFICIAL_PACKAGES.map(x=>({...x,localState:'VAULT_NOT_CONFIGURED'}));res.json({authority:'مجمع الملك فهد لطباعة المصحف الشريف',authorityTrust:'PRIMARY_OFFICIAL_AUTHORITY',officialCertification:'CERTIFIED',localUsePolicy:'DIRECT_AFTER_OFFICIAL_CHECKSUM_AND_STRUCTURE',catalog})});
  app.get('/api/science/quran/kfgqpc/audio-catalog',sensitiveIdentityRateLimit,requireGovernanceRoles(['scientific_admin','super_admin','org_admin','comp_admin','auditor']),(req,res)=>res.json({authority:'مجمع الملك فهد لطباعة المصحف الشريف',authorityTrust:'PRIMARY_OFFICIAL_AUTHORITY',catalog:KFGQPC_OFFICIAL_AUDIO,note:'KFGQPC published recordings are accepted as official reference sources. Operational playback remains bound to the exact ingested asset, hash and reading scope.'}));
  app.get('/api/science/quran/kfgqpc/developer-assets',sensitiveIdentityRateLimit,requireGovernanceRoles(['scientific_admin','super_admin','org_admin','comp_admin','auditor']),(req,res)=>res.json({authority:'مجمع الملك فهد لطباعة المصحف الشريف',authorityTrust:'PRIMARY_OFFICIAL_AUTHORITY',officialCertification:'CERTIFIED',quranPackages:KFGQPC_OFFICIAL_PACKAGES,additionalAssets:KFGQPC_DEVELOPER_ASSETS}));
  app.get('/api/science/quran/kfgqpc/library',sensitiveIdentityRateLimit,requireGovernanceRoles(['scientific_admin','super_admin','org_admin','comp_admin','head_judge','auditor']),(req,res)=>{const state=serverQuranSources?(id:string)=>serverQuranSources!.localState(id):undefined;res.json({summary:kfgqpcLibrarySummary(state),items:buildKfgqpcOfficialLibrary(state)});});
  ...
  app.get('/api/science/quran/kfgqpc/delivery-status',sensitiveIdentityRateLimit,requireGovernanceRoles(['scientific_admin','super_admin','org_admin','comp_admin','head_judge','auditor']),(req,res)=>res.json(kfgqpcDelivery.status()));
  app.get('/api/science/quran/kfgqpc/page/:packageId/:page',sensitiveIdentityRateLimit,requireGovernanceRoles(['scientific_admin','super_admin','org_admin','comp_admin','head_judge','judge','auditor']),async(req,res)=>{const packageId=safeSegment(String(req.params.packageId||'')),page=Number(req.params.page);if(!Number.isInteger(page)||page<1||page>700)return res.status(400).json({code:'MUSHAF_PAGE_INVALID'});try{const asset=await kfgqpcDelivery.page(packageId,page);if(await sendKfgqpcAsset(res,asset,'private, max-age=3600'))return;return res.status(404).json({code:'OFFICIAL_MUSHAF_PAGE_ASSET_NOT_INGESTED'})}catch{return res.status(502).json({code:'OFFICIAL_MUSHAF_PAGE_DELIVERY_FAILED'})}});
   ...
   */
  app.post('/api/judging/blind-rescoring/plan',requireGovernanceRoles(['head_judge','scientific_admin','comp_admin','org_admin']),(req,res)=>{
    const b=req.body||{};
  ...
  app.post('/api/judging/blind-rescoring/measure',requireGovernanceRoles(['head_judge','scientific_admin','comp_admin','org_admin','auditor']),(req,res)=>{
    const b=req.body||{};
  ...
  app.post('/api/results/seal/verify',requireGovernanceRoles(['comp_admin','org_admin','head_judge','auditor','scientific_admin']),(req,res)=>{

...
import { claimsFromGrant, writeIdentityClaims, claimsWritable } from './server/firebase-claims';
...
async function startServer() {
  ...
  app.post('/api/identity/activate',requireFirebaseBase,async(req,res)=>{if(!identityGovernance)return res.status(503).json({code:'IDENTITY_GOVERNANCE_NOT_CONFIGURED'});const base=(req as any).firebaseBase as {uid:string;email?:string};try{const result=identityGovernance.activate(base,String(req.body?.activationToken||''));

...
async function startServer() {
  ...
  app.post('/api/identity/invitations',requireGovernanceRoles(['super_admin','operator_owner','org_admin','comp_admin']),(req,res)=>{if(!identityGovernance)return res.status(503).json({code:'IDENTITY_GOVERNANCE_NOT_CONFIGURED'});try{const requestedRole=String(req.body?.requestedRole||'') as GovernanceRole;const operatorId=req.body?.operatorId?String(req.body.operatorId):undefined;if(['operator_owner','operator_admin'].includes(requestedRole)&&(!operatorId||!saasPlatform?.operatorExists(operatorId)))throw new Error('OPERATOR_NOT_FOUND');res.status(201).json(identityGovernance.createInvitation((req as any).mizanIdentity,{email:String(req.body?.email||''),displayName:String(req.body?.displayName||''),requestedRole,operatorId,organizationId:req.body?.organizationId?String(req.body.organizationId):undefined,competitionId:req.body?.competitionId?String(req.body.competitionId):undefined,committeeId:req.body?.committeeId?String(req.body.committeeId):undefined,reason:String(req.body?.reason||'')}))}catch(err){res.status(400).json({code:err instanceof Error?err.message:'INVITATION_FAILED'})}});
  app.post('/api/identity/invitations/:id/approve',requireGovernanceRoles(['super_admin','operator_owner','org_admin','comp_admin']),(req,res)=>{if(!identityGovernance)return res.status(503).json({code:'IDENTITY_GOVERNANCE_NOT_CONFIGURED'});try{res.json(identityGovernance.approveInvitation((req as any).mizanIdentity,String(req.params.id)))}catch(err){res.status(400).json({code:err instanceof Error?err.message:'APPROVAL_FAILED'})}});
  app.post('/api/identity/accounts/:id/suspend',requireGovernanceRoles(['super_admin','operator_owner','org_admin','comp_admin']),(req,res)=>{if(!identityGovernance)return res.status(503).json({code:'IDENTITY_GOVERNANCE_NOT_CONFIGURED'});try{res.json(identityGovernance.suspend((req as any).mizanIdentity,String(req.params.id),String(req.body?.reason||'')))}catch(err){res.status(400).json({code:err instanceof Error?err.message:'SUSPEND_FAILED'})}});
  app.delete('/api/identity/accounts/:id',requireGovernanceRoles(['super_admin','operator_owner','org_admin','comp_admin']),(req,res)=>{if(!identityGovernance)return res.status(503).json({code:'IDENTITY_GOVERNANCE_NOT_CONFIGURED'});try{res.json(identityGovernance.remove((req as any).mizanIdentity,String(req.params.id),String(req.body?.reason||'User removed by authorized administrator')))}catch(err){res.status(400).json({code:err instanceof Error?err.message:'REMOVE_FAILED'})}});
  app.post('/api/identity/grants/:id/suspend',requireGovernanceRoles(['super_admin','operator_owner','org_admin','comp_admin']),(req,res)=>{if(!identityGovernance)return res.status(503).json({code:'IDENTITY_GOVERNANCE_NOT_CONFIGURED'});try{res.json(identityGovernance.suspendGrant((req as any).mizanIdentity,String(req.params.id),String(req.body?.reason||'')))}catch(err){res.status(400).json({code:err instanceof Error?err.message:'GRANT_SUSPEND_FAILED'})}});
  app.post('/api/identity/grants/:id/resume',requireGovernanceRoles(['super_admin','operator_owner','org_admin','comp_admin']),(req,res)=>{if(!identityGovernance)return res.status(503).json({code:'IDENTITY_GOVERNANCE_NOT_CONFIGURED'});try{res.json(identityGovernance.resumeGrant((req as any).mizanIdentity,String(req.params.id),String(req.body?.reason||'')))}catch(err){res.status(400).json({code:err instanceof Error?err.message:'GRANT_RESUME_FAILED'})}});
  app.post('/api/identity/grants/:id/reissue-qr',requireGovernanceRoles(['super_admin','operator_owner','org_admin','comp_admin']),(req,res)=>{if(!identityGovernance)return res.status(503).json({code:'IDENTITY_GOVERNANCE_NOT_CONFIGURED'});try{res.json(identityGovernance.reissueQr((req as any).mizanIdentity,String(req.params.id)))}catch(err){res.status(400).json({code:err instanceof Error?err.message:'QR_REISSUE_FAILED'})}});
  app.delete('/api/identity/grants/:id',requireGovernanceRoles(['super_admin','operator_owner','org_admin','comp_admin']),(req,res)=>{if(!identityGovernance)return res.status(503).json({code:'IDENTITY_GOVERNANCE_NOT_CONFIGURED'});try{res.json(identityGovernance.removeGrant((req as any).mizanIdentity,String(req.params.id),String(req.body?.reason||'Competition access removed by authorized administrator')))}catch(err){res.status(400).json({code:err instanceof Error?err.message:'GRANT_REMOVE_FAILED'})}});
  app.post('/api/identity/grants/:id/revoke-sessions',requireGovernanceRoles(['super_admin','operator_owner','org_admin','comp_admin','head_judge']),(req,res)=>{if(!identityGovernance)return res.status(503).json({code:'IDENTITY_GOVERNANCE_NOT_CONFIGURED'});try{res.json(identityGovernance.revokeGrantSessions((req as any).mizanIdentity,String(req.params.id),String(req.body?.reason||'')))}catch(err){res.status(400).json({code:err instanceof Error?err.message:'SESSION_REVOKE_FAILED'})}});
  app.post('/api/identity/accounts/:id/revoke-sessions',requireGovernanceRoles(['super_admin','operator_owner','org_admin','comp_admin','head_judge']),(req,res)=>{if(!identityGovernance)return res.status(503).json({code:'IDENTITY_GOVERNANCE_NOT_CONFIGURED'});try{res.json(identityGovernance.revokeSessions((req as any).mizanIdentity,String(req.params.id),String(req.body?.reason||'')))}catch(err){res.status(400).json({code:err instanceof Error?err.message:'SESSION_REVOKE_FAILED'})}});
  ...
  const resolvedManagedIdentity=(uid:string):ServerIdentity|null=>{const managed=identityGovernance?.identityForUid(uid);if(!managed)return null;return {uid:managed.account.uid,email:managed.account.email,role:managed.grant.role,organizationId:managed.grant.organizationId,operatorId:managed.grant.operatorId||saasPlatform?.operatorIdForOrganization(managed.grant.organizationId),competitionId:managed.grant.competitionId}};
  const assertNotificationTarget=(actor:ServerIdentity,target:NotificationTarget)=>{
    ...
    const assertUser=(uid:string)=>{const identity=resolvedManagedIdentity(uid);if(!identity)throw new Error('NOTIFICATION_TARGET_USER_NOT_FOUND');if(globalSender)return;if(['operator_owner','operator_admin'].includes(actor.role)){if(!actor.operatorId||identity.operatorId!==actor.operatorId)throw new Error('CROSS_OPERATOR_ACCESS_BLOCKED');return}if(identity.organizationId!==actor.organizationId)throw new Error('CROSS_TENANT_ACCESS_BLOCKED');if(actor.role==='comp_admin'&&actor.competitionId&&identity.competitionId&&identity.competitionId!==actor.competitionId)throw new Error('CROSS_COMPETITION_ACCESS_BLOCKED')};
  ...
  app.get('/api/notifications',requireGovernanceRoles(Array.from(governanceRoles)),(req,res)=>{if(!notificationCenter)return res.status(503).json({code:'NOTIFICATION_CENTER_NOT_CONFIGURED'});try{return res.json(notificationCenter.list((req as any).mizanIdentity))}catch(err){return res.status(400).json({code:err instanceof Error?err.message:'NOTIFICATION_LIST_FAILED'})}});
  app.post('/api/notifications',sensitiveIdentityRateLimit,requireGovernanceRoles(Array.from(notificationSenders)),(req,res)=>{if(!notificationCenter)return res.status(503).json({code:'NOTIFICATION_CENTER_NOT_CONFIGURED'});try{const actor=(req as any).mizanIdentity as ServerIdentity;const target={...(req.body?.target||{})} as NotificationTarget;assertNotificationTarget(actor,target);return res.status(201).json(notificationCenter.publish(actor,{title:String(req.body?.title||''),body:String(req.body?.body||''),category:req.body?.category,priority:req.body?.priority,target,senderName:String(req.body?.senderName||''),actionHref:req.body?.actionHref?String(req.body.actionHref):undefined,actionLabel:req.body?.actionLabel?String(req.body.actionLabel):undefined,dedupeKey:req.body?.dedupeKey?String(req.body.dedupeKey):undefined,expiresAt:req.body?.expiresAt?String(req.body.expiresAt):undefined}))}catch(err){return res.status(400).json({code:err instanceof Error?err.message:'NOTIFICATION_SEND_FAILED'})}});
  app.post('/api/notifications/read-all',requireGovernanceRoles(Array.from(governanceRoles)),(req,res)=>{if(!notificationCenter)return res.status(503).json({code:'NOTIFICATION_CENTER_NOT_CONFIGURED'});try{return res.json(notificationCenter.markAllRead((req as any).mizanIdentity))}catch(err){return res.status(400).json({code:err instanceof Error?err.message:'NOTIFICATION_UPDATE_FAILED'})}});

...
test('runtime health only promotes question escrow when server explicitly reports configured',()=>{
 const yes=normalizeRuntimeHealth({status:'ok',questionEscrowConfigured:true,secureQuestionRuntimeConfigured:true,serverQuranSourceVaultConfigured:true,passSigningConfigured:true});

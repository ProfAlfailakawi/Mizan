...
test('delegated tenant admins can provision competition staff directly and stay tenant-scoped',()=>{
 assert.equal(canGrantRole('org_admin','judge'),true);assert.equal(roleGrantRequiresDualApproval('judge'),false);
 assert.equal(validateRoleGrant({actorRole:'org_admin',targetRole:'judge',actorId:'a',approverId:'a',organizationId:'o1',targetOrganizationId:'o1'}).ok,true);
...
test('server identity invitation is immediately ready, one-time, and creates scoped named account',()=>{
 const dir=fs.mkdtempSync(path.join(os.tmpdir(),'mizan-id-'));try{const repo=new IdentityGovernanceRepository(dir);const created=repo.createInvitation(orgAdmin('admin-a'),{email:'judge@example.org',displayName:'Judge A',requestedRole:'judge',competitionId:'comp-1',committeeId:'committee-1',reason:'Official judging assignment'});assert.equal(created.invitation.status,'READY');assert.ok(created.activationToken);const activated=repo.activate({uid:'firebase-judge',email:'judge@example.org'},created.activationToken!);assert.equal(activated.grant.role,'judge');assert.equal(activated.grant.committeeId,'committee-1');assert.throws(()=>repo.activate({uid:'firebase-judge-2',email:'judge@example.org'},created.activationToken!),/ACTIVATION_TOKEN_INVALID|IDENTITY_ALREADY_BOUND/);const audit=repo.audit(orgAdmin('admin-a'));assert.ok(audit.some(x=>x.action==='IDENTITY_ACTIVATED'));assert.equal(repo.verifyAudit('org-1').valid,true);}finally{fs.rmSync(dir,{recursive:true,force:true})}
});

test('privileged identity cannot silently open on two different devices and admin can revoke the old session',()=>{
 const dir=fs.mkdtempSync(path.join(os.tmpdir(),'mizan-id-'));try{const repo=new IdentityGovernanceRepository(dir);const inv=repo.createInvitation(orgAdmin('a'),{email:'judge@example.org',displayName:'Judge',requestedRole:'judge',competitionId:'comp-1',reason:'Official panel assignment'});repo.activate({uid:'judge-uid',email:'judge@example.org'},inv.activationToken!);const identity={uid:'judge-uid',email:'judge@example.org',role:'judge' as const,organizationId:'org-1',competitionId:'comp-1'};const first=repo.openSession(identity,'tablet-1','Judge tablet','MFA');assert.equal(first.status,'ACTIVE');assert.equal(first.authenticationAssurance,'MFA');assert.throws(()=>repo.openSession(identity,'tablet-2','Replacement'),/PRIVILEGED_SESSION_CONFLICT/);const account=repo.identityForUid('judge-uid')!.account;assert.equal(repo.revokeSessions(orgAdmin('a'),account.id,'Secure replacement device handover').count,1);assert.equal(repo.openSession(identity,'tablet-2','Replacement').status,'ACTIVE');}finally{fs.rmSync(dir,{recursive:true,force:true})}
});

...
export interface NotificationRecord{
 ...
 dedupeKey?:string;expiresAt?:string;
}
...
export class NotificationCenterRepository{
 ...
 list(identity:ServerIdentity){
  ...
  const notifications=s.notifications.filter(n=>(!n.expiresAt||Date.parse(n.expiresAt)>t)&&this.matches(identity,n.target)).map(n=>{const state=s.userStates.find(x=>x.userId===identity.uid&&x.notificationId===n.id);return {...n,readAt:state?.readAt,archivedAt:state?.archivedAt}}).filter(n=>!n.archivedAt).sort((a,b)=>{const priority={urgent:0,important:1,normal:2};return priority[a.priority]-priority[b.priority]||String(b.createdAt).localeCompare(String(a.createdAt))});
  return {notifications,unread:notifications.filter(x=>!x.readAt).length};
 }
 publish(actor:{uid:string;role:string},input:{title:string;body:string;category?:NotificationCategory;priority?:NotificationPriority;target:NotificationTarget;senderName?:string;actionHref?:string;actionLabel?:string;dedupeKey?:string;expiresAt?:string}){
  ...
  const s=this.read();const dedupeKey=clean(input.dedupeKey,180)||undefined;
 ...
 markAllRead(identity:ServerIdentity){const s=this.read();const visible=s.notifications.filter(n=>this.matches(identity,n.target));const at=now();for(const n of visible){const row=this.stateFor(s,identity.uid,n.id);if(!row.archivedAt)row.readAt=at}this.write(s);return {count:visible.length,readAt:at}}

...
import {QuranSessionEvidenceStore} from './quran-session-evidence';
...
export class QuranIntelligenceService{
  ...
  readonly evidence:QuranSessionEvidenceStore;
  ...
  constructor(private root:string,private quran:ServerQuranSourceRepository,private alignmentBackend:QuranAlignmentBackendConfig={}){
    fs.mkdirSync(root,{recursive:true,mode:0o700});this.vector=new QuranVectorRepository(path.join(root,'vector'));this.knowledge=new QuranKnowledgeRepository(path.join(root,'knowledge'));this.benchmarks=new QuranAlignmentBenchmarkRepository(path.join(root,'benchmarks'));this.evidence=new QuranSessionEvidenceStore(path.join(root,'session-evidence'));this.bootstrapOfficialWaqf();
  }
  capabilities(){return {protocol:'MIZAN-QURAN-INTELLIGENCE-2',authority:'KFGQPC',readings:QURAN_READINGS.map(r=>r.id),spatialMapping:true,vectorMetadata:true,waqfLayer:{enabled:true,mode:'AUTO_DERIVE_FROM_CERTIFIED_KFGQPC_AYA_TEXT',wordIndexPolicy:'NEVER_INFER'},tajweedLayer:true,streamingAlignment:{mode:'SHADOW_ONLY',backendConfigured:!!this.alignmentBackend.url,scoreAuthority:'HUMAN_ONLY',canAffectScore:false,evidenceTimeline:true,evidenceReplay:true,silentShadowJudge:true},storage:{masterOriginals:'OFFLINE_ONLY',heavyAssets:'PRIVATE_R2_OR_LOCAL_CACHE',frontendPrivateUrlAccess:false}}}
  ...
  health(){const readings=this.status().map((x:any)=>{const layers={spatial:x.spatial==='VERIFIED',waqf:x.waqf?.status==='VERIFIED',waqfScience:x.waqfScience?.status==='VERIFIED',tajweed:x.tajweed?.status==='VERIFIED',vector:x.vector?.status==='VERIFIED'&&Number(x.vector?.verifiedWordMappings||0)>0,alignment:x.alignmentBenchmark?.status==='VERIFIED'&&x.alignmentBenchmark?.passed===true&&!!this.alignmentBackend.url};const ready=Object.values(layers).filter(Boolean).length;return {reading:x.reading,sourcePackage:x.sourcePackage,layers,ready,total:Object.keys(layers).length,blocked:Object.entries(layers).filter(([,v])=>!v).map(([k])=>k),provenance:{spatial:x.coverage?.sourceVersion,waqf:x.waqf?.sourceVersion,waqfScience:x.waqfScience?.sourceVersion,tajweed:x.tajweed?.sourceVersion,vector:x.vector?.sourceVersion,alignmentModel:x.alignmentBenchmark?.modelVersion}}});return {protocol:'MIZAN-QURAN-HEALTH-1',authority:'KFGQPC',generatedAt:new Date().toISOString(),alignmentBackendConfigured:!!this.alignmentBackend.url,readings,summary:{readyLayers:readings.reduce((n:any,r:any)=>n+r.ready,0),totalLayers:readings.reduce((n:any,r:any)=>n+r.total,0),fullyReadyReadings:readings.filter((r:any)=>r.ready===r.total).length}}}
  readingGuard(input:{reading:string;sourcePackageId:string}){const id=readingId(input.reading),def=quranReadingDefinition(id)!;const conflicts:string[]=[];if(input.sourcePackageId!==def.packageId)conflicts.push('SOURCE_PACKAGE_READING_MISMATCH');const waqf=this.knowledge.loadWaqf(id),science=this.knowledge.loadWaqfScience(id),tajweed=this.knowledge.loadTajweed(id),vector=this.vector.load(id);if(waqf&&waqf.reading!==id)conflicts.push('WAQF_READING_MISMATCH');if(science&&science.reading!==id)conflicts.push('WAQF_SCIENCE_READING_MISMATCH');if(tajweed&&tajweed.reading!==id)conflicts.push('TAJWEED_READING_MISMATCH');if(vector&&vector.reading!==id)conflicts.push('VECTOR_READING_MISMATCH');return {reading:id,expectedSourcePackage:def.packageId,providedSourcePackage:input.sourcePackageId,status:conflicts.length?'BLOCKED':'CLEAR',conflicts,crossReadingFallback:'FORBIDDEN' as const}}
  sessionEvidence(actorId:string,sessionId:string){if(!actorId.trim()||!sessionId.trim())throw new Error('QURAN_ALIGNMENT_SESSION_IDENTITY_REQUIRED');return this.evidence.get(actorId,sessionId)||{protocol:'MIZAN-QURAN-SESSION-EVIDENCE-1',sessionId,actorId,startedAt:'',updatedAt:'',events:[],summary:{alignmentEvents:0,recoveryEvents:0,humanMarkers:0,lostEvents:0,reacquiredEvents:0},integrity:{sha256:''}}}
  markHumanEvent(actorId:string,sessionId:string,eventType:string){if(!eventType.trim())throw new Error('QURAN_HUMAN_MARKER_REQUIRED');return this.evidence.appendHumanMarker({actorId,sessionId,eventType})}

...
export class QuranIntelligenceService{
  ...
  passage(input:{reading:string;surah:unknown;startAyah:unknown;endAyah:unknown}){
    ...
    const resolved=this.quran.resolvePassage({packageId:def.packageId,surah,startAyah,endAyah});const pageLoci=this.quran.resolvePassageLoci({packageId:def.packageId,surah,startAyah,endAyah});
  ...
  registerWaqf(x:WaqfDataset){return this.knowledge.registerWaqf(x)}
  ...
  async processAlignmentChunk(input:{actorId:string;sessionId:string;reading:string;surah:unknown;startAyah:unknown;endAyah:unknown;sourcePackageId:string;contentType:string;bytes:Buffer}){
    ...
    let visualLocation=null,wordVector=null,waqfEvidence:unknown[]=[];if(output.ayah){visualLocation=this.quran.canonicalLocation({reading:id,surah:output.surah!,ayah:output.ayah});if(output.wordIndex)wordVector=this.vector.resolveWord({reading:id,surah:output.surah!,ayah:output.ayah,wordIndex:output.wordIndex});waqfEvidence=this.knowledge.waqfForAyah(id,output.surah!,output.ayah).filter(x=>!output.wordIndex||!x.wordIndex||x.wordIndex===output.wordIndex)}
    return {...output,visualLocation,wordVector,waqfEvidence,backendEvidence:{modelVersion:backendModelVersion,acousticQuality:observation.acousticQuality},scoreAuthority:'HUMAN_ONLY' as const,scoreDelta:0 as const,shadowMode:true as const};
  }

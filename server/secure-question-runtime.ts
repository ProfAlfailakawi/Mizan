...
export class SecureQuestionRuntimeRepository {
  private diversity:QuestionDiversityLedger;
  constructor(private root:string,private quran:ServerQuranSourceRepository,private pools:ServerQuestionPoolRepository,private escrow:QuestionEscrowRepository,diversity?:QuestionDiversityLedger){fs.mkdirSync(root,{recursive:true,mode:0o700});this.diversity=diversity||new QuestionDiversityLedger(path.join(root,'diversity-ledger'))}
  ...
  provision(input:RuntimeProvisionInput){
    ...
    const secret=crypto.randomBytes(32).toString('base64url'),readingKey=[input.qiraah,input.rawi,input.tariq||''].join('|');

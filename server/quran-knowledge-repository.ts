...
export function validateWaqfDataset(input:WaqfDataset):WaqfDataset{
  if(input.version!=='MIZAN-KFGQPC-WAQF-1')throw new Error('WAQF_DATASET_VERSION_INVALID');assertReading(input.reading);validateDatasetProvenance(input.provenance);
...
export class QuranKnowledgeRepository{
  ...
  waqfStatus(reading:QuranReadingId){const x=this.loadWaqf(reading);return x?{status:x.provenance.status,occurrences:x.occurrences.length,sourceVersion:x.provenance.sourceVersion}:{status:'OFFICIAL_DATA_UNAVAILABLE' as const,occurrences:0}}

...
export function validateFirebaseBaseClaims(payload:JwtPayload,projectId:string,nowSeconds=Math.floor(Date.now()/1000)):FirebaseBaseIdentity{
  ...
  return {uid:payload.sub,email:typeof payload.email==='string'?payload.email:undefined,raw:payload};
}
export function firebaseSecondFactorPresent(raw:Record<string,unknown>){
  const firebase=raw.firebase;
  if(!firebase||typeof firebase!=='object')return false;
  const factor=(firebase as Record<string,unknown>).sign_in_second_factor;
  return typeof factor==='string'?factor.trim().length>0:!!factor;
}
...
async function verifyFirebaseTokenSignature(token:string,projectId:string){const [head,body,sig,...rest]=String(token||'').split('.');if(!head||!body||!sig||rest.length)throw new Error('FIREBASE_TOKEN_MALFORMED');const header=decodeJson<JwtHeader>(head);if(header.alg!=='RS256'||!header.kid)throw new Error('FIREBASE_TOKEN_ALGORITHM_INVALID');const payload=decodeJson<JwtPayload>(body);const certs=await googleCerts();const cert=certs[header.kid];if(!cert)throw new Error('FIREBASE_SIGNING_KEY_UNKNOWN');const valid=crypto.verify('RSA-SHA256',Buffer.from(`${head}.${body}`),cert,Buffer.from(sig,'base64url'));if(!valid)throw new Error('FIREBASE_SIGNATURE_INVALID');return payload;}
export async function verifyFirebaseBaseIdToken(token:string,projectId:string){const payload=await verifyFirebaseTokenSignature(token,projectId);return validateFirebaseBaseClaims(payload,projectId);}
export async function verifyFirebaseIdToken(token:string,projectId:string){const payload=await verifyFirebaseTokenSignature(token,projectId);return validateFirebaseClaims(payload,projectId);}

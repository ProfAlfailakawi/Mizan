...
export default function App() {
 ...
 if(requireAuth&&accessError) return <div className="min-h-screen grid place-items-center bg-[#f7f5ef] p-5"><div className="mizan-surface p-7 max-w-md text-center"><div className="mizan-kicker">ACCESS GOVERNANCE</div><h1 className="text-xl font-black mt-2">{accessError==='MFA_REQUIRED'?'يلزم تحقق بخطوتين لهذا الدور':accessError==='PRIVILEGED_SESSION_CONFLICT'?'الحساب مفتوح على جهاز حساس آخر':'الحساب غير مفوض'}</h1><p className="text-xs text-[#737a75] mt-3 leading-6">{accessError==='MFA_REQUIRED'?'لأن هذا الحساب يستطيع التأثير في مسابقة عالية الحساسية، لا يسمح MIZAN بالدخول الأحادي. فعّل المصادقة متعددة العوامل لدى موفر الهوية ثم أعد تسجيل الدخول.':accessError==='PRIVILEGED_SESSION_CONFLICT'?'منع MIZAN جلسة متزامنة لهذا الدور. اطلب من مدير المسابقة إغلاق الجلسة القديمة إذا كان الجهاز السابق مفقودًا أو متعطلًا.':'الهوية صحيحة، لكن الحساب يحتاج دعوة وصلاحية محددة داخل المؤسسة قبل الدخول.'}</p>{accessError==='ACCOUNT_NOT_PROVISIONED'&&<div className="mt-5 text-start"><label className="text-[10px] font-black text-[#707873]">رمز التفعيل لمرة واحدة</label><input value={activationToken} onChange={e=>setActivationToken(e.target.value)} className="mizan-input mt-2" placeholder="Activation token"/><button onClick={()=>void activateAccount()} className="mt-3 w-full rounded-xl bg-[#214C40] text-white py-2.5 text-xs font-black">ربط هذا الحساب بالدعوة</button>{activationMessage&&<div className="mt-2 text-[10px] text-center text-[#7b827d]">{activationMessage}</div>}</div>}<div className="text-[10px] text-[#8a908c] mt-3">{accessError}</div><button onClick={()=>signOut(auth)} className="mt-5 text-xs font-bold text-[#214C40]">تسجيل الخروج / Sign out</button></div></div>;

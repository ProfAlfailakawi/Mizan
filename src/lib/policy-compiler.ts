...
import { certifiedCapabilityFor, sourceUsableForCompetition, isKfgqpcOfficialReading } from './scientific-core';
...
export function detectContradictions(input:{competition:Competition;quranSources:QuranSourceManifestRecord[];aiValidations:AICapabilityValidationRecord[];availableQualifiedJudges:number;committeeCount?:number;committeeSeesDelegation?:boolean;certificateProofRequired?:boolean}){
 ...
 for(const cat of c.categories){
  ...
  if(!source&&!isKfgqpcOfficialReading({riwaya:cat.riwaya}))push({severity:'BLOCKER',kind:'scientific',title:`No certified Quran source for ${cat.riwaya}`,titleAr:`لا يوجد مصدر قرآني معتمد لرواية ${cat.riwaya}`,why:'Official passages must resolve to an immutable certified source for the exact reading.',whyAr:'يجب أن ترجع المواضع الرسمية إلى مصدر معتمد غير قابل للتعديل لنفس الرواية.',affectedWorkflow:'FairDraw / JudgeOS',flowAr:'السحب العادل / منصة المحكم',evidence:[cat.riwaya],fixTarget:'scientific'});
  if(p.judging.silentAiGuardian&&!certifiedCapabilityFor(input.aiValidations,{capability:'word_alignment',riwaya:cat.riwaya}))push({severity:'REVIEW',kind:'qiraah_ai',title:`AI unavailable for ${cat.riwaya}`,titleAr:`قدرة الذكاء الاصطناعي غير معتمدة لرواية ${cat.riwaya}`,why:'The competition remains operational, but this AI capability is not certified for the reading.',whyAr:'المسابقة تعمل بكامل طاقتها، لكن هذه القدرة غير معتمدة لهذه الرواية.',affectedWorkflow:'Post-lock AI review',flowAr:'مراجعة الذكاء بعد القفل',evidence:[cat.riwaya,'word_alignment'],fixTarget:'scientific'});
 }

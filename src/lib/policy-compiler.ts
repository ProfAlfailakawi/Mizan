...
export function detectContradictions(input:{competition:Competition;quranSources:QuranSourceManifestRecord[];aiValidations:AICapabilityValidationRecord[];availableQualifiedJudges:number;committeeCount?:number;committeeSeesDelegation?:boolean;certificateProofRequired?:boolean}){
 const c=input.competition,p=getCompetitionPolicy(c);const issues:ContradictionIssueRecord[]=[];/*
 * المالك الافتراضي هو الجهة.
 *
 * إغفالُه لا يعني «لا أحد»: أخطرُ الحالتين أن يُخفى عن الجهة عطبٌ تملك إصلاحه، فالافتراض
 * هو الإظهار. وبندُ المنصّة يُسمّى صراحةً، فإخفاؤه قرارٌ مكتوب لا سهوٌ في حقل.
 */
 const push=(x:Omit<ContradictionIssueRecord,'id'|'competitionId'>)=>issues.push({id:`conflict-${issues.length+1}`,competitionId:c.id,owner:'organizer',...x});
 if(p.judging.identityVisibility==='code_only'&&input.committeeSeesDelegation)push({severity:'BLOCKER',kind:'blind_judging',title:'Blind judging conflicts with delegation visibility',titleAr:'التحكيم المحايد يتعارض مع ظهور بيانات الوفد',why:'Identity-linked delegation data can reveal the participant during independent judging.',whyAr:'بيانات الوفد المرتبطة بالهوية قد تكشف المتسابق أثناء التحكيم المستقل.',affectedWorkflow:'JudgeOS',flowAr:'منصة المحكم',evidence:['identityVisibility=code_only','committeeSeesDelegation=true'],fixTarget:'competition_dna'});
 ...
 for(const cat of c.categories){
  ...
  /* فئةٌ بلا نطاقٍ محسوم: الجلسة لا تبدأ، والعطل يظهر أول مرة أمام المحكّم لا قبله. */
  if(scopeAyahCount(categoryScopeOf(cat))===0)push({severity:'BLOCKER',kind:'scientific',title:`Category ${cat.name} has no decided scope`,titleAr:`الفئة ${cat.nameArabic||cat.name} بلا نطاقٍ محسوم`,why:'A legacy juz count does not say which juz, so no session can start for this category.',whyAr:'عددُ الأجزاء الموروث لا يحدد أيّ أجزاء، فلا تبدأ جلسةٌ لهذه الفئة.',affectedWorkflow:'FairDraw / JudgeOS',flowAr:'السحب العادل / منصة المحكم',evidence:[cat.memorizationScope||'',`juzCount=${cat.juzCount||0}`].filter(Boolean),fixTarget:'competition_dna'});
  /* نصٌّ موجود لكن جسر المواضع ناقص: السحب منه يعطي موضعًا لا وجود له في الرواية. */
  if(!categoryReadingsQuestionSafe(cat.riwaya)){const unsafe=categoryUnsafeReadings(cat.riwaya);const rawiId=unsafe[0]||resolveCanonicalRawiId({riwaya:cat.riwaya});push({severity:'BLOCKER',kind:'scientific',title:`Incomplete canonical-to-native locus crosswalk for ${cat.riwaya}`,titleAr:`جسر المواضع غير مكتمل لرواية ${cat.riwaya}`,why:'A drawn canonical locus cannot be resolved to this reading own ayah numbering, so the passage shown would not be the passage asked.',whyAr:'الموضع القانوني المسحوب لا يُحلّ إلى ترقيم هذه الرواية، فالمقطع المعروض غير المقطع المطلوب.',affectedWorkflow:'FairDraw / JudgeOS',flowAr:'السحب العادل / منصة المحكم',evidence:rawiId?readingQuestionBlockers(rawiId):[cat.riwaya],fixTarget:'competition_dna',owner:'platform'})}
  if(!source&&!isReadingDelivered({riwaya:cat.riwaya}))push({severity:'BLOCKER',kind:'scientific',title:`No certified Quran source for ${cat.riwaya}`,titleAr:`لا يوجد مصدر قرآني معتمد لرواية ${cat.riwaya}`,why:'Official passages must resolve to an immutable certified source for the exact reading.',whyAr:'يجب أن ترجع المواضع الرسمية إلى مصدر معتمد غير قابل للتعديل لنفس الرواية.',affectedWorkflow:'FairDraw / JudgeOS',flowAr:'السحب العادل / منصة المحكم',evidence:[cat.riwaya],fixTarget:'competition_dna',owner:'platform'});
  if(p.judging.silentAiGuardian&&!certifiedCapabilityFor(input.aiValidations,{capability:'word_alignment',riwaya:cat.riwaya}))push({severity:'REVIEW',kind:'qiraah_ai',title:`AI unavailable for ${cat.riwaya}`,titleAr:`قدرة الذكاء الاصطناعي غير معتمدة لرواية ${cat.riwaya}`,why:'The competition remains operational, but this AI capability is not certified for the reading.',whyAr:'المسابقة تعمل بكامل طاقتها، لكن هذه القدرة غير معتمدة لهذه الرواية.',affectedWorkflow:'Post-lock AI review',flowAr:'مراجعة الذكاء بعد القفل',evidence:[cat.riwaya,'word_alignment'],fixTarget:'none',owner:'platform'});
 }
 if(input.certificateProofRequired&&!p.certificates.enabled)push({severity:'REVIEW',kind:'certificate',title:'Proof policy requires certificates but issuance is disabled',titleAr:'سياسة الإثبات تتطلب شهادات لكن الإصدار متوقف',why:'Public proof cannot be attached to a certificate that is never issued.',whyAr:'لا يمكن ربط إثبات عام بشهادة لا تُصدر أبدًا.',affectedWorkflow:'Certificate lifecycle',flowAr:'دورة حياة الشهادة',evidence:['certificate proof required','certificates.enabled=false'],fixTarget:'competition_dna'});
 if(p.judging.silentAiGuardian&&p.aiPolicy.mode!=='AI_DISABLED'&&!p.privacy.allowAiProcessing)push({severity:'BLOCKER',kind:'privacy',title:'AI policy conflicts with privacy consent policy',titleAr:'سياسة الذكاء الاصطناعي تتعارض مع سياسة خصوصية الموافقة',why:'AI processing is enabled operationally while competition privacy policy prohibits AI processing.',whyAr:'معالجة الذكاء الاصطناعي مفعّلة تشغيليًا بينما تمنعها سياسة خصوصية المسابقة.',affectedWorkflow:'Post-lock AI review',flowAr:'مراجعة الذكاء بعد القفل',evidence:[`aiMode=${p.aiPolicy.mode}`,'privacy.allowAiProcessing=false'],fixTarget:'privacy'});
 if(p.results.visibility==='ceremony_only'&&p.results.publicLeaderboard!=='disabled')push({severity:'BLOCKER',kind:'publication',title:'Ceremony-only results conflict with public leaderboard',titleAr:'نتائج «الحفل فقط» تتعارض مع لوحة الترتيب العامة',why:'A public leaderboard can disclose results before the protected ceremony reveal.',whyAr:'لوحة الترتيب العامة قد تكشف النتائج قبل إعلانها في الحفل.',affectedWorkflow:'Results / Ceremony',flowAr:'النتائج / الحفل',evidence:['visibility=ceremony_only',`publicLeaderboard=${p.results.publicLeaderboard}`],fixTarget:'competition_dna'});
 if(p.certificates.publicVerification&&!p.certificates.enabled)push({severity:'REVIEW',kind:'certificate',title:'Public certificate verification is enabled while certificates are disabled',titleAr:'التحقق العام من الشهادات مفعّل بينما إصدار الشهادات متوقف',why:'The verifier cannot authenticate certificates that policy never permits issuing.',whyAr:'لا يستطيع المدقّق التحقق من شهادات لا تسمح السياسة بإصدارها.',affectedWorkflow:'Public verifier',flowAr:'المدقّق العام',evidence:['publicVerification=true','certificates.enabled=false'],fixTarget:'competition_dna'});
 return issues;
}

...
async function getJson<T>(url:string):Promise<T>{const token=await bearer();if(token === 'demo-token') return {} as T; const r=await fetch(url,{headers:{authorization:`Bearer ${token}`,accept:'application/json'},cache:'no-store'});const body=await r.json().catch(()=>({}));if(!r.ok)throw new Error(String(body.code||`HTTP_${r.status}`));return body as T}

...
export function buildPreflight(input:{competition:Competition;policy:CompetitionPolicy;integrations:IntegrationConfig[];devices:DeviceRecord[];judges:JudgeProfile[];committees:Committee[];quranSources:QuranSourceManifestRecord[];aiValidations?:AICapabilityValidationRecord[];backups:BackupRecord[];isOffline:boolean;questionEscrowAssurance?:'development_client_gate'|'operational_panel_gate'|'production_server_escrow';entitlementIssue?:string}){
 const {competition:c,policy:p,integrations,devices,judges,committees,quranSources,aiValidations=[],backups,isOffline,questionEscrowAssurance='operational_panel_gate',entitlementIssue=''}=input;
 ...
 const checks:PreflightCheck[]=[
  ...
  /* حدُّ الترخيص يُعرف قبل اليوم لا فيه: الخادم رفض قياسًا، فيُقال هنا بدل أن يُبتلع. */
  check('entitlements',!entitlementIssue,false,'حدود الترخيص','License entitlements',`الجهة بلغت حدًّا في ترخيصها (${entitlementIssue||'—'})، فقد تُرفض إضافة متسابقين أو فتح مسابقة أخرى.`,`The organization reached a licensed limit (${entitlementIssue||'—'}); adding participants or opening another competition may be refused.`,'none'),

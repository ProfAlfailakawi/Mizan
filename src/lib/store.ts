...
import { createScopeEngineActions } from './store-scope-actions';
...
let publicProjectionSyncTimer: ReturnType<typeof setTimeout> | null = null;
function markCompetitionConfigChanged(){
  const now=new Date().toISOString();
  globalState.competitionConfigUpdatedAt=now;
  schedulePublicCompetitionProjectionSync();
  return now;
}
...
/*
 * صفحة التسجيل ليست لقطةً تُنشأ يوم فتح التسجيل ثم تُنسى. الفئات وسياسة التسجيل
 * والنطاقات قد تُعدّل بعد الفتح؛ إبقاء الإسقاط العام قديمًا كان يُظهر للطالب فئاتٍ حُذفت
 * أو يخفي فئاتٍ أضيفت. كل تغيير إعداد بعد النشر يحدّث الإسقاط العام، مع debounce حتى لا
 * تتحول الكتابة في حقل نصي إلى عشرات عمليات الشبكة.
 */
function schedulePublicCompetitionProjectionSync(){
  if(typeof window==='undefined'||globalState.isOffline||!auth.currentUser||launchPlaceholderActive())return;
  if(!['super_admin','org_admin','comp_admin'].includes(globalState.currentUser.role))return;
  if(['draft','configured'].includes(globalState.competition.status))return;
  if(publicProjectionSyncTimer)clearTimeout(publicProjectionSyncTimer);
  publicProjectionSyncTimer=setTimeout(()=>{
    publicProjectionSyncTimer=null;
    void (async()=>{
      try{
        const updatedAt=globalState.competitionConfigUpdatedAt||new Date().toISOString();
        const {db,doc,setDoc}=await getFirestoreClient();
        await setDoc(doc(db,'public_competitions',globalState.competition.id),{organizationId:globalState.competition.organizationId,competition:globalState.competition,updatedAt});
        /* مرآة القرص تُبقي بيئة الخادم غير المتصلة بفايرستور متسقة أيضًا. */
        try{
          const token=await auth.currentUser?.getIdToken();
          if(token)await fetch(`/api/public/competitions/${encodeURIComponent(globalState.competition.id)}/publish`,{method:'POST',headers:{authorization:`Bearer ${token}`,'content-type':'application/json'},body:JSON.stringify({organizationId:globalState.competition.organizationId,competition:globalState.competition})});
        }catch{/* فايرستور هو المصدر الأول؛ فشل المرآة لا يعيد النسخة العامة إلى الوراء. */}
        resolveCloudScope('public_competitions');
      }catch(err){reportCloudError(classifyCloudError(err),'public_competitions');}
    })();
  },450);
}
 ...
 */
type ServerAuditRefusal={status:number;code:string;count:number;firstAt:string;lastAt:string};
const auditToken=(value:unknown,max:number)=>String(value??'').toUpperCase().replace(/[^A-Z0-9_]/g,'').slice(0,max);
/*
 * واسمُ الحدث يبقى في الذاكرة ولا يُكتب.
 *
 * الفحصُ الأمني تتبّع أرقامَ الشهادات ومعرّفاتها وهي تسري إلى صفّ التدقيق، فكتابةُ أيّ
 * حقلٍ من ذلك الصفّ إلى تخزين المتصفّح تضع بياناتٍ لا حاجة إليها حيث لا تُحمى. والدائمُ
 * هو ما يكفي وحده: الرمزُ والحالةُ والعددُ والوقت — ولا شيء منها من جسم الحدث.
 *
 * والاسمُ يفيد المشغّلَ في جلسته، فيبقى في الذاكرة مقصورًا على رمزٍ معروف الشكل، ويذهب
 * بذهابها.
 */
const refusedActionsThisSession:string[]=[];
...
function recordServerAuditRefusal(row:ServerAuditMirror,status:number,code:string){
 const action=auditToken(row.action,64)||'UNNAMED';
 if(!refusedActionsThisSession.includes(action)){refusedActionsThisSession.push(action);if(refusedActionsThisSession.length>50)refusedActionsThisSession.shift()}
 // لا يُكتب إلى التخزين إلا ما لم يأتِ من جسم الحدث: الرمزُ من ردّ الخادم، والحالةُ رقم.
 try{const named=auditToken(code,64)||`HTTP_${status}`;const at=new Date().toISOString();
  const rows=readServerAuditRefusals();const found=rows.find(x=>x.code===named&&x.status===status);
  if(found){found.count+=1;found.lastAt=at}else rows.push({status,code:named,count:1,firstAt:at,lastAt:at});
  localStorage.setItem(SERVER_AUDIT_REFUSED_KEY,JSON.stringify(rows.slice(-200)))}catch{}
}
/** يُقرأ في الواجهة والاختبار: ما رفضه الخادمُ رفضًا دائمًا وبقي في السجلّ المحلّي وحده. */
export function serverAuditRefusals(){return {counts:readServerAuditRefusals(),actionsThisSession:[...refusedActionsThisSession]}}
...
export function useAppStore() {
  ...
  const syncUserId=state.currentUser.id;
  const syncOrganizationId=state.competition.organizationId;
  const syncCompetitionId=state.competition.id;
  ...
  useEffect(() => {
    ...
    try {
      ...
      const orgId = syncOrganizationId, compId = syncCompetitionId;
  ...
  }, [syncUserId,syncOrganizationId,syncCompetitionId]);
  ...
  const refreshQueueNotifications=()=>{
    const competitionId=globalState.competition.id;
    for(const c of globalState.committees.filter(committee=>committee.competitionId===competitionId)){
      const q=globalState.participants
        .filter(p=>p.competitionId===competitionId&&p.assignedCommitteeId===c.id&&p.status==='in_queue')
        .sort((a,b)=>queueOrderValue(a)-queueOrderValue(b));
      if(q[0])appendParticipantNotifications(q[0],'queue.next');
      if(q[1])appendParticipantNotifications(q[1],'queue.prepare');
    }
  };
   ...
   */
  /*
   * زمن الجلسة يُقاس من ساعة الحائط لا من عدّاد.
   *
   * العدّاد يتوقف مع نوم الجهاز أو تبديل التبويب، فيخرج زمنٌ أقصر من الحقيقة ويُكتب في
   * السجلّ على أنه الحقيقة. والفارق بين لحظتين لا يتوقف. و`carriedSeconds` يحفظ ما
   * تراكم قبل استعادةٍ من نقطة استمرارية، فلا تبتلع الاستعادة ما مضى.
   */
  const sessionElapsedSeconds = () => {
    const a = globalState.activeSession;
    const carried = a.carriedSeconds || 0;
    if(!a.startedAt) return carried || a.durationSeconds || 0;
    const started = Date.parse(a.startedAt);
    if(!Number.isFinite(started)) return carried || a.durationSeconds || 0;
    return carried + Math.max(0, Math.floor((Date.now() - started) / 1000));
  };
  ...
  const checkInParticipant = (participantIdOrCode: string, method: 'kiosk_qr' | 'mobile_self' | 'exception_host' = 'kiosk_qr') => {
    ...
    const found = findByIdOrCode(activeRoster, participantIdOrCode);
    const pIndex = found ? globalState.participants.findIndex(x => x.id === found.id && x.competitionId===globalState.competition.id) : -1;
    ...
    if (pIndex !== -1) {
      ...
      if (!unrouted && updated.assignedCommitteeId) {
        const promisedPanel = globalState.committees.find(c => c.id === updated.assignedCommitteeId && c.competitionId === updated.competitionId);
        if (promisedPanel) {
      ...
      syncParticipantLifecycle(updated);
  ...
  const recordAIObservation = (input: Omit<AIObservation,'id'|'competitionId'>) => {
    ...
    const participant=globalState.participants.find(p=>p.id===(input.participantId||globalState.activeSession.participant?.id)&&p.competitionId===globalState.competition.id);
  ...
  const recordJudgeEvent = (type: JudgeEventType) => {
    ...
    const newEvent: JudgeEvent = {
      ...
      timestamp: new Date().toISOString(),
      relativeSeconds: sessionElapsedSeconds(),
      type,
  ...
  const lockAndSubmitAssessment = (directScores?: Record<string, number>) => {
    ...
    if (participant && sessionSubs.length >= sessionRuleSet.judgesCountPerPanel) {
      ...
      if (existing < 0) {
        ...
        const sessionMinutes=Math.round(sessionElapsedSeconds()/60*10)/10;
  ...
  const nextQuestion = () => {
    ...
    if(total>0&&globalState.activeSession.currentQuestionIndex<total-1){
      ...
      notify();
      /*
       * إن ورث الموضعُ الجديدُ توثيقَ ما قبله فهو مكشوف من لحظته، وطورُ الجلسة يتبع ذلك.
       *
       * ولولا هذا لبقيت الجلسة «مختومة» بينما السؤال ظاهر، فلا يعمل «إنهاء الموضع» —
       * وهو يشترط طور التلاوة. والحجز يتقدّم إلى «مكشوف» هنا كما يتقدّم بعد الموافقة
       * اليدوية: ما سُمع في القاعة لا يعود إلى المخزون.
       */
      void (async()=>{
        const index=globalState.activeSession.currentQuestionIndex;
        const g=await ensureQuestionRevealGate(index);
        if(!g||g.status!=='REVEALED')return;
        if(globalState.activeSession.currentQuestionIndex!==index)return;
        globalState.activeSession.isReciting=true;
        globalState.activeSession.questionPhase='RECITING';
        const item=globalState.activeSession.questionSelection?.questions[index];
        if(item){
          const held=globalState.questionReservations.filter(r=>r.sessionId===globalState.activeSession.sessionId&&r.questionId===item.id).map(r=>r.id);
          if(held.length)advanceReservations(held,'revealed',`كُشف السؤال ${index+1} في القاعة`);
        }
        auditTrustAction('QUESTION_REVEALED_AFTER_PANEL_APPROVAL','QuestionRevealGate',g.id,`كشف السؤال ${index+1} بتوثيق الجلسة نفسها: المتسابق حاضر واللجنة وافقت عند الموضع الأول`,`Revealed question ${index+1} on the session's existing verification: participant present and panel approved at the first passage`);
        notify();
      })();
    }
  };
  ...
  const completeCompetition = () => {
    ...
    markCompetitionConfigChanged();
    const certificatePolicy=getCompetitionPolicy(globalState.competition).certificates;
    if(certificatePolicy.enabled&&certificatePolicy.autoIssue){
      for(const result of globalState.results.filter(r=>r.competitionId===globalState.competition.id&&['sealed','published'].includes(r.status)))void generateCertificate(result.id);
    }
    void syncPublicJourneys();
    notify(); return true;
  };
  ...
  const generateCertificate = async (resultId: string) => {
    ...
    const newCert: Certificate = {
      ...
      competitionName: storedCompetitionName(true), competitionNameArabic: storedCompetitionName(false),
      organizationName: globalState.organization.name || globalState.organization.brand?.name || '', organizationNameArabic: globalState.organization.nameArabic || globalState.organization.brand?.nameArabic || globalState.organization.name || '',
    ...
    if(proof)void (async()=>{
      const state=await publishCertificateToRegistry({
        ...
        competitionName:globalState.competition.nameArabic||globalState.competition.name,organizationName:globalState.organization.nameArabic||globalState.organization.name,
    ...
    const certParticipant=globalState.participants.find(x=>x.id===res.participantId&&x.competitionId===globalState.competition.id); if(certParticipant){appendParticipantNotifications(certParticipant,'certificate.ready');void publishPublicJourneyRecord(certParticipant);}
    notify();
    return newCert;
  };
  ...
  const registerParticipant = (newP: Omit<Participant, 'id' | 'code' | 'status' | 'statusHistory' | 'createdAt'>) => {
    ...
    if (shouldAutoApprove) {
      const idx = globalState.participants.findIndex(p => p.id === participant.id && p.competitionId === participant.competitionId);
    ...
    const finalParticipant=globalState.participants.find(p => p.id === participant.id && p.competitionId === participant.competitionId)!;
  ...
  const removeParticipant = (participantId: string) => {
    const target = globalState.participants.find(p=>p.id===participantId&&p.competitionId===globalState.competition.id);
    if(!target) return false;
    // A participant who already sat before a panel keeps a scored record; deleting would orphan it.
    if(['in_session','tested','certified','appealed'].includes(target.status) || globalState.results.some(r=>r.competitionId===globalState.competition.id&&r.participantId===participantId)) return false;
    globalState.participants = globalState.participants.filter(p=>!(p.id===participantId&&p.competitionId===globalState.competition.id));
  ...
  const submitAppeal = (participantId: string, grounds: AppealRecord['grounds'], reasonText: string) => {
    ...
    void persistScopedDocument('appeals',appeal.id,appeal as unknown as Record<string,unknown>);
    const pIdx = globalState.participants.findIndex(p => p.id === participantId && p.competitionId===globalState.competition.id);
    if (pIdx >= 0) { const appealed={ ...globalState.participants[pIdx], status: 'appealed' as const, statusHistory: [...globalState.participants[pIdx].statusHistory, { status:'appealed' as const, timestamp:new Date().toISOString(), actor:'Participant portal', reason:grounds }] }; globalState.participants[pIdx]=appealed; syncParticipantLifecycle(appealed); }
  ...
  /*
   * خبرٌ يُقال وليس فشلًا يُعامَل معاملته.
   *
   * استبدالُ رمزٍ كان عند صاحبه وحده **نجاحٌ** تُفتح به البطاقة وتُصدَّر: لو عُدَّ فشلًا
   * لأسقطته دفعةُ التصدير، فعاد العطب من بابٍ آخر. لكنه خبرٌ يعني المنظّم، فيُنقل منفصلًا.
   */
  let journeyAccessNotice='';
  const lastJourneyAccessNotice=()=>journeyAccessNotice;
  const ensureParticipantJourneyAccess=async(participantId:string)=>{
    journeyAccessFailure='';journeyAccessNotice='';
    ...
    /*
     * ما سُلّم لصاحبه وحده تُصدره الجهة من الخادم، ولا يُسكت عنه بخطأ.
     *
     * التسجيل العام لا يُبقي الرمز عند الجهة ولا في السحابة، فبقيت الجهة تُمنع من فتح QR
     * لمن سجّل من الرابط العام — وهم أكثر المتسابقين — وتُنصح بانتظار مزامنةٍ لا تأتي.
     * والخادم وحده يُصدر رمزًا بالصيغة المعتمدة ويُبطل سابقه، فيُطلب منه بدل توليدٍ محليّ
     * يترك رمزين حيّين لبابٍ واحد.
     */
    if(journeyTokenHeldByHolderOnly(current)&&!current.journeyAccessToken){
      const issued=await reissueParticipantJourneyAccess(participantId);
      if(!issued){journeyAccessFailure=globalState.isOffline?'OFFLINE':!auth.currentUser?'NOT_SIGNED_IN':'HOLDER_TOKEN_REISSUE_FAILED';return null;}
      const at=globalState.participants.findIndex(p=>p.id===participantId);
      if(at>=0){globalState.participants[at]={...globalState.participants[at],journeyTokenCustody:'organizer'};void persistScopedDocument('participants',participantId,globalState.participants[at] as unknown as Record<string,unknown>);notify();}
      journeyAccessNotice='HOLDER_TOKEN_REPLACED';journeyAccessFailure='';
      return globalState.participants[at>=0?at:idx];
    }
...
const prepareJourneyAccessBatch=async()=>{
  ...
  /*
   * مسابقة التهيئة ليست سجلًا حقيقيًا، لكنها في نسخ أقدم كانت قابلة للتحرير كأنها مسابقة.
   * إذا وصل مدير الجهة إلى النشر ومعه عمل محفوظ على هذا المعرّف المؤقت، لا نطلب منه إعادة
   * بناء المسابقة ولا ننشر المعرّف المحظور. نُرقّي المسودة مرة واحدة إلى هوية حقيقية، وننقل
   * معها كل سجل محلي يحمل competitionId المؤقت قبل أول كتابة سحابية.
   */
  const materializePendingCompetitionForPublish = (): {ok:boolean;reason:string} => {
    if(globalState.competition.id!=='comp-pending-setup')return {ok:true,reason:''};
    const organizationId=globalState.competition.organizationId!=='org-pending-setup'
      ? globalState.competition.organizationId
      : globalState.organization.id!=='org-pending-setup'?globalState.organization.id:'';
    if(!organizationId)return {ok:false,reason:'اربط المسابقة بجهة حقيقية أولًا؛ جهة التهيئة المؤقتة لا يمكن نشرها للعامة.'};
    if(!can(globalState.currentUser.role,'competition.create'))return {ok:false,reason:'هذه مسودة تهيئة قديمة. اطلب من مدير الجهة إنشاء هوية نهائية للمسابقة ثم أعد النشر.'};
    const previousId='comp-pending-setup';
    const nextId=newId('comp');
    const updatedAt=new Date().toISOString();
    const nextCompetition:Competition={...globalState.competition,id:nextId,organizationId,updatedAt};
    ...
    for(const [key,value] of Object.entries(mutable)){
      ...
      mutable[key]=value.map(item=>{
        ...
        if(row.competitionId!==previousId)return item;
        return {...row,competitionId:nextId,...(('organizationId' in row)&&(!row.organizationId||row.organizationId==='org-pending-setup')?{organizationId}: {})};
      });
    }
    globalState.competitions=[nextCompetition,...globalState.competitions.filter(c=>c.id!==previousId)];
    globalState.competition=nextCompetition;
    if(globalState.currentUser.competitionId===previousId)globalState.currentUser={...globalState.currentUser,competitionId:nextId,organizationId};
    globalState.competitionConfigUpdatedAt=updatedAt;
    notify();
  ...
  /*
   * إجراءات محرك النطاق تعيش في وحدةٍ مستقلة (store-scope-actions).
   *
   * كانت هنا داخل الخطّاف، فلا تُختبر في Node إلا عبر الشاشة — والشاشة ترى ما يُعرض لا ما
   * صار في الحالة. فصارت مصنعًا يأخذ مضيفه، والمخزن أحد مضيفيه، والاختبار مضيفٌ آخر.
   */
  const scopeEngine = createScopeEngineActions({
    getState: () => globalState,
    newId,
    notify,
    audit: (action, entityType, entityId, ar, en) => auditTrustAction(action, entityType as Parameters<typeof auditTrustAction>[1], entityId, ar, en),
    createIncident: (kind, title, description, severity) =>
      createIncident(kind as Parameters<typeof createIncident>[0], title, description, severity as Parameters<typeof createIncident>[3]),
    markConfigChanged: markCompetitionConfigChanged,
    queueScopeUpload,
  });
  const {
  ...
  const removeCategory = (categoryId: string) => {
    if (globalState.participants.some(p=>p.competitionId===globalState.competition.id&&p.categoryId===categoryId)) return false;
  ...
  const addCommittee = () => {
    const n=globalState.committees.filter(c=>c.competitionId===globalState.competition.id).length+1;
  ...
  const updateCommittee = (committeeId: string, patch: Partial<Committee>) => {
    globalState.committees=globalState.committees.map(c=>c.id===committeeId&&c.competitionId===globalState.competition.id?{...c,...patch}:c);
    const next=globalState.committees.find(c=>c.id===committeeId&&c.competitionId===globalState.competition.id);
  ...
  const updateCommitteeJudgeSpecialty = (committeeId: string, judgeId: string, specialty: string) => {
    const committee=globalState.committees.find(c=>c.id===committeeId&&c.competitionId===globalState.competition.id);
    if(!committee)return;
  ...
  const removeCommittee = (committeeId:string) => {
    ...
    const hasParticipantHistory=globalState.participants.some(p=>p.competitionId===globalState.competition.id&&p.assignedCommitteeId===committeeId);
    ...
    if(hasHistory){
      ...
      globalState.committees=globalState.committees.map(c=>c.id===committeeId&&c.competitionId===globalState.competition.id?paused:c);
  ...
  const publishPublicCompetitionRecord=async():Promise<{ok:boolean;reason:string}>=>{
    ...
    /* إن كانت مسودة التهيئة القديمة تحمل إعدادات حقيقية، حوّلها إلى هوية نهائية بعد اجتياز حراس الطلب وقبل حارس المعرّف المؤقت. */
    const materialized=materializePendingCompetitionForPublish();
    if(!materialized.ok)return materialized;
    if(launchPlaceholderActive())return {ok:false,reason:'اربط المسابقة بجهة حقيقية أولًا؛ جهة التهيئة المؤقتة لا يمكن نشرها للعامة.'};
    try{
      const updatedAt=new Date().toISOString();
      const publishedCompetition:Competition={...globalState.competition,updatedAt};
      globalState.competition=publishedCompetition;
      globalState.competitions=globalState.competitions.map(c=>c.id===publishedCompetition.id?publishedCompetition:c);
      globalState.competitionConfigUpdatedAt=updatedAt;
      const configPersistResult=await persistCompetitionConfiguration(updatedAt);
      ...
      const {db,doc,setDoc,getDoc}=await getFirestoreClient();
      const publicRef=doc(db,'public_competitions',publishedCompetition.id);
      await setDoc(publicRef,{organizationId:publishedCompetition.organizationId,competition:publishedCompetition,updatedAt});
      /* لا نعلن النجاح بمجرد انتهاء write: اقرأ المرجع العام نفسه وتأكد أن الهوية والمراجعة وصلتا. */
      const verification=await getDoc(publicRef);
      const verified=verification.exists()?verification.data() as {organizationId?:unknown;competition?:Competition;updatedAt?:unknown}:null;
      if(!verified||verified.competition?.id!==publishedCompetition.id||verified.organizationId!==publishedCompetition.organizationId||verified.updatedAt!==updatedAt){
        return {ok:false,reason:'تم إرسال النشر لكن تعذّر التحقق من النسخة العامة بعد الكتابة. لم نعلن الرابط منشورًا؛ أعد المحاولة بعد لحظات.'};
      }
      try{
        const token=await auth.currentUser.getIdToken();
        const mirror=await fetch(`/api/public/competitions/${encodeURIComponent(publishedCompetition.id)}/publish`,{
          method:'POST',
          headers:{authorization:`Bearer ${token}`,'content-type':'application/json'},
          cache:'no-store',
          body:JSON.stringify({organizationId:publishedCompetition.organizationId,competition:publishedCompetition})
        });
        if(!mirror.ok)console.warn('MIZAN public competition mirror rejected publish',{status:mirror.status,competitionId:publishedCompetition.id});
      }catch(serverErr){
        /* Firestore هو المصدر السلطوي للتسجيل؛ المرآة الخادمية احتياط ولا يجوز أن تصنع نجاحًا كاذبًا أو فشلًا كاذبًا. */
  ...
  const checkPublicCompetitionPublished=async():Promise<{state:'published'|'missing'|'unknown';reason:string}>=>{
    ...
    if(globalState.competition.organizationId==='org-pending-setup')return {state:'missing',reason:'اربط المسابقة بجهة حقيقية أولًا؛ جهة التهيئة المؤقتة لا يمكن نشرها للعامة.'};
    if(globalState.competition.id==='comp-pending-setup')return {state:'missing',reason:'هذه مسودة تهيئة قديمة محفوظة. اضغط «انشر الآن» وسيحوّلها ميزان إلى مسابقة بهوية نهائية ويحفظ عملك قبل النشر.'};
    let firestoreReached=false;
    try{
      const {db,doc,getDoc}=await getFirestoreClient();
      const snap=await getDoc(doc(db,'public_competitions',globalState.competition.id));
      firestoreReached=true;
      const data=snap.exists()?snap.data() as {organizationId?:unknown;competition?:Competition;updatedAt?:unknown}:null;
      const expectedRevision=globalState.competitionConfigUpdatedAt||globalState.competition.updatedAt;
      const published=!!data?.competition&&data.competition.id===globalState.competition.id&&data.organizationId===globalState.competition.organizationId&&(!expectedRevision||data.updatedAt===expectedRevision);
    ...
    try{
      const resp=await fetch(`/api/public/competitions/${encodeURIComponent(globalState.competition.id)}`,{cache:'no-store'});
      /* 200 و404 فقط يجيبان سؤال «هل النسخة موجودة؟». خطأ 5xx لا يُحوَّل إلى «غير منشور». */
      serverDefinitive=resp.ok||resp.status===404;
      if(resp.ok){
        const payload=await resp.json();
        const expectedRevision=globalState.competitionConfigUpdatedAt||globalState.competition.updatedAt;
        if(payload?.competition&&payload.competition.id===globalState.competition.id&&payload.organizationId===globalState.competition.organizationId&&(!expectedRevision||payload.updatedAt===expectedRevision)){
  ...
  const startSessionForParticipant = async (participantId: string) => {
    ...
    const assignedCommittee = participant ? globalState.committees.find(c => c.id === participant.assignedCommitteeId && c.competitionId === participant.competitionId) : undefined;
    ...
    if(productionMode){
      try{
        ...
        globalState.activeSession={sessionId:runtime.sessionId,participant,committee,questionSelection:null,currentQuestionIndex:0,isReciting:false,startedAt:new Date().toISOString(),carriedSeconds:0,durationSeconds:0,events:[],isLocked:false,audioLevel:76,questionPhase:'SEALED',secureQuestionMode:'SERVER',secureRuntimeSessionId:runtime.sessionId,secureQuestionCount:runtime.questionCount};
    ...
    try {
      ...
      globalState.activeSession = {
        sessionId, participant, committee, questionSelection:selection, currentQuestionIndex:0, isReciting:false, startedAt:new Date().toISOString(), carriedSeconds:0, durationSeconds:0, events:[], isLocked:false, audioLevel:76, questionPhase:'SEALED',secureQuestionMode:'CLIENT'
      };
  ...
  const ensureQuestionRevealGate=async(questionIndex=globalState.activeSession.currentQuestionIndex)=>{
    ...
    if(gate)return gate;
    /*
     * البوابة للموضع الأول من الجلسة، لا لكل موضع فيها.
     *
     * غرضُها أن يُفتح السؤال والمتسابق **حاضر** وباللجنة **إذن**. وذلك يُستوثق مرة واحدة
     * عند بدء الجلسة؛ فإذا انتهى موضع وجاء الذي بعده فالمتسابق لم يبرح مكانه ولا اللجنة
     * تبدّلت. فإعادة السؤال «أثبت حضوره ثم وافق» بين كل موضعين احتكاكٌ في قاعةٍ لا تنتظر،
     * ولا يضيف ضمانة: يقولها المحكّم نفسه عن المتسابق نفسه في الجلسة نفسها.
     *
     * فيُورَّث التوثيق عن موضعٍ كُشف فعلًا في هذه الجلسة — عن كشفٍ وقع، لا عن نيّة كشف —
     * ويبقى الأول وحده هو الذي يُسأل. وإن لم يُكشف أولٌ بعدُ فالبوابة مغلقة كما كانت.
     */
    const opened=globalState.questionRevealGates
      .filter(g=>g.sessionId===session.sessionId&&g.questionIndex<questionIndex&&g.status==='REVEALED'&&g.participantPresence.verified)
      .sort((a,b)=>b.questionIndex-a.questionIndex)[0];
    const required=[...new Set(committee.judgeIds)];
    const inherited=opened?{presence:opened.participantPresence,approvals:opened.approvals.filter(a=>required.includes(a.judgeId))}:null;
    /* اللجنة قد تتبدّل بين موضعين، فالنصاب يُعاد حسابه على أعضائها الآن لا على من وافق أمس. */
    const revealPolicyNow=getCompetitionPolicy(globalState.competition).questions.secureReveal||{requireParticipantPresence:true,judgeApprovalMode:'all_assigned' as const};
    const inheritedReady=inherited&&questionRevealReady({participantPresent:inherited.presence.verified,requiredJudgeIds:required,approvals:inherited.approvals,mode:revealPolicyNow.judgeApprovalMode,minimumApprovals:revealPolicyNow.minimumApprovals}).ready;
    gate={id:newId('qgate'),competitionId:globalState.competition.id,sessionId:session.sessionId,participantId:participant.id,committeeId:committee.id,questionIndex,participantPresence:inherited?inherited.presence:{verified:false},requiredJudgeIds:required,approvals:inherited?inherited.approvals:[],status:inheritedReady?'REVEALED':'SEALED',revealedAt:inheritedReady?new Date().toISOString():undefined,createdAt:new Date().toISOString(),questionCommitmentHash:await hashCanonical({questionSetId:session.questionSelection?.questionSetId,questionIndex,questionId:q.id,quranSourcePackageHash:session.questionSelection?.quranSourcePackageHash||'DEVELOPMENT'}),quranSourcePackageHash:session.questionSelection?.quranSourcePackageHash,revealAssurance:productionMode?'operational_panel_gate':'development_client_gate'};
  ...
  const addParticipantPassportEntry=(participantId:string)=>{
    const competitionId=globalState.competition.id;
    const p=globalState.participants.find(x=>x.id===participantId&&x.competitionId===competitionId);if(!p)return;
    const r=globalState.results.find(x=>x.competitionId===competitionId&&x.participantId===participantId);
    const c=globalState.certificates.find(x=>x.competitionId===competitionId&&x.participantId===participantId);
    const cat=globalState.competition.categories.find(x=>x.id===p.categoryId);
    const e:ParticipantPassportEntry={id:newId('pp'),participantId,competitionId,competitionName:storedCompetitionName(true),categoryName:cat?.name||'',year:globalState.competition.startDate.slice(0,4),result:r?`${r.rank} / ${r.finalScore}`:undefined,certificateNumber:c?.certificateNumber,verified:!!c};
    globalState.participantPassport=[e,...globalState.participantPassport.filter(x=>!(x.participantId===participantId&&x.competitionId===competitionId))];notify();return e;
  };
  const addJudgePassportEntry=(judgeId:string)=>{
    const competitionId=globalState.competition.id;
    const j=globalState.judges.find(x=>(x.id===judgeId||x.userId===judgeId)&&(!x.competitionId||x.competitionId===competitionId));if(!j)return;
    const participantIds=new Set(globalState.participants.filter(p=>p.competitionId===competitionId).map(p=>p.id));
    const completedSessions=globalState.judgeSubmissions.filter(x=>x.judgeId===j.userId&&(!x.participantId||participantIds.has(x.participantId))).length;
    const e:JudgePassportEntry={id:newId('jp'),judgeId:j.id,competitionId,competitionName:storedCompetitionName(true),role:j.specialty,riwayat:j.certifiedRiwayat,calibrationScore:j.calibrationScore,completedSessions,verified:j.isReady};
    globalState.judgePassport=[e,...globalState.judgePassport.filter(x=>!(x.judgeId===j.id&&x.competitionId===competitionId))];notify();return e;
  };
  ...
  const optimizeArrivalSlots = () => {
    ...
    changed.forEach(p=>{appendParticipantNotifications(p,'arrival.updated');syncParticipantLifecycle(p)});
    notify();
  ...
  const getFairnessReceipt = (participantId:string) => {
    ...
    if(!p) return null;
    const r=globalState.results.find(x=>x.competitionId===globalState.competition.id&&x.participantId===participantId);
    const sub=globalState.judgeSubmissions.filter(x=>x.participantId===participantId);
    const cert=globalState.certificates.find(x=>x.competitionId===globalState.competition.id&&x.participantId===participantId);
  ...
  const getQueueEstimate=(participantId:string)=>{
    ...
    const raw=p?globalState.committees.find(x=>x.id===p.assignedCommitteeId&&x.competitionId===p.competitionId):undefined;
    const c=raw&&measured[raw.id]?{...raw,averageSessionMinutes:measured[raw.id]}:raw;
    return estimateQueueWait({
      participant:p,
      committee:c,
      committeeQueueInOrder:c?globalState.participants.filter(x=>x.competitionId===globalState.competition.id&&x.status==='in_queue'&&x.assignedCommitteeId===c.id).sort((a,b)=>queueOrderValue(a)-queueOrderValue(b)):[],
      fallbackSessionMinutes:globalState.competition.ruleSet.questionDurationMinutes,
      now:Date.now(),
    });
  };
  ...
  const transferQueueParticipants=(input:{sourceCommitteeId:string;targetCommitteeId:string;participantIds?:string[];mode:QueueTransferMode;reason:string;allowCrossCategory?:boolean})=>{
     ...
     */
    const now=new Date();
    const scopedParticipants=globalState.participants.filter(p=>p.competitionId===globalState.competition.id);
    const preselected=(input.participantIds?.length
      ? scopedParticipants.filter(p=>input.participantIds!.includes(p.id)&&p.status==='in_queue'&&p.assignedCommitteeId===source.id)
      : scopedParticipants.filter(p=>p.status==='in_queue'&&p.assignedCommitteeId===source.id));
    const targetQueue=scopedParticipants.filter(p=>p.status==='in_queue'&&p.assignedCommitteeId===target.id);
    ...
    const plan=planQueueTransfer({participants:scopedParticipants,sourceCommitteeId:source.id,targetCommitteeId:target.id,participantIds:input.participantIds,mode:input.mode,equityOrderKeys});if(!plan.ok)return plan;
    const invalid=plan.selectedIds.map(id=>scopedParticipants.find(p=>p.id===id)).filter((p):p is Participant=>!!p).filter(p=>!compatibleCommitteesFor(p).some(c=>c.id===target.id));
    /*
    ...
    globalState.participants=globalState.participants.map(p=>{if(p.competitionId!==globalState.competition.id)return p;const c=byId.get(p.id);if(!c)return p;
    ...
    for(const id of plan.selectedIds){const p=globalState.participants.find(x=>x.id===id&&x.competitionId===globalState.competition.id);if(p){appendParticipantNotifications(p,`queue.transferred:${stamp}`);syncParticipantLifecycle(p);}}
  ...
  const applyDistributionPlan = async (plan:DistributionPlan):Promise<{ok:boolean;reason:string;assigned?:number}> => {
    ...
    const check = await verifyDistributionPlan(plan, async () => {
      const pool = globalState.participants.filter(p => ids.includes(p.id) && p.competitionId === globalState.competition.id && p.status === 'in_queue');
      return planDistribution({
        competitionId: globalState.competition.id,
        participants: pool,
        committees: committeesAtMeasuredTempo(),
    ...
    globalState.participants=globalState.participants.map(p=>{
      if(p.competitionId!==globalState.competition.id)return p;
      ...
      assigned++;
      const target=globalState.committees.find(c=>c.id===row.committeeId&&c.competitionId===globalState.competition.id);
    ...
    for(const id of [...byParticipant.keys()]){const p=globalState.participants.find(x=>x.id===id&&x.competitionId===globalState.competition.id);if(p){appendParticipantNotifications(p,`queue.transferred:${stamp}`);syncParticipantLifecycle(p);}}
    auditTrustAction(
  ...
  const recommendCommitteeElasticity=()=>{
    ...
    const load=(c:Committee)=>globalState.participants.filter(p=>p.competitionId===globalState.competition.id&&p.status==='in_queue'&&p.assignedCommitteeId===c.id).length*Math.max(1,c.averageSessionMinutes);
    ...
    const recommendation=recommendBalancedQueueMove({participants:globalState.participants.filter(p=>p.competitionId===globalState.competition.id),sourceCommittee:source,targetCommittee:target,maxMove:5,isCompatible:(p,t)=>compatibleCommitteesFor(p).some(c=>c.id===t.id)});if(!recommendation.participantIds.length)return null;
...
globalState.competition=next;globalState.competitions=globalState.competitions.map(c=>c.id===next.id?next:c);const publishedAt=new Date().toISOString();globalState.policyCompilations=globalState.policyCompilations.map(x=>x.id===id?{...x,state:'PUBLISHED',publishedAt,publishedGenomeVersion:getCompetitionPolicy(next).version}:x);auditTrustAction('POLICY_COMPILER_PUBLISHED','CompetitionGenome',next.id,'نشر نسخة Genome جديدة بعد مراجعة بشرية ومحاكاة بلا موانع','Published a new Genome version only after human review and blocker-free simulation');notify();return true;};
  ...
  const runOperationalRehearsal=async()=>{
    ...
    const readyJudges=globalState.judges.filter(j=>j.isReady&&(!j.competitionId||j.competitionId===globalState.competition.id));const activeCommittees=globalState.committees.filter(c=>c.competitionId===globalState.competition.id&&c.status!=='offline');const sampleParticipant=globalState.participants.find(p=>p.competitionId===globalState.competition.id&&!['rejected','certified'].includes(p.status));
  ...
  const verifyActiveFairDrawProof=async()=>{const sel=globalState.activeSession.questionSelection;const p=globalState.activeSession.participant;
    ...
    const proof=globalState.fairDrawProofs.find(x=>x.questionSetId===globalState.activeSession.questionSelection?.questionSetId);if(proof)return verifyFairDrawPublicProof(proof);if(productionMode)return {valid:false,reason:'PRODUCTION_SERVER_PROOF_REQUIRED'} as const;if(!sel||!p)return {valid:false,reason:'NO_SELECTION'} as const;const source=sel.quranSourceManifestId?globalState.quranSourceManifests.find(q=>q.id===sel.quranSourceManifestId):undefined;const content=source?globalState.quranSourceContents.find(c=>c.sourceManifestId===source.id&&c.packageHash===source.packageHash):undefined;const resolution=participantEffectiveScope(p.id);const category=globalState.competition.categories.find(c=>c.id===p.categoryId);const pool=source&&content?sourceResolvedQuestionPool(p,source,content):(resolution&&!resolution.blocked?buildCandidatePool({scope:resolution.scope,category,reading:readingContextOf({riwaya:p.riwaya})}).map(q=>({id:q.id,surahNumber:q.surahNumber,surahNameArabic:surahNameArabic(q.surahNumber),surahNameEnglish:'',startAyah:q.startAyah,endAyah:q.endAyah,juzNumber:q.juzNumber||1,riwaya:p.riwaya,expectedTextArabic:'',difficultyRating:q.difficultyRating,mutashabihatDensity:'none' as const,tajweedComplexity:q.tajweedComplexity||'intermediate',timesUsed:q.priorUsageCount||0})):[]);return verifyFairDrawSelection({selection:sel,pool,participant:p,policy:getCompetitionPolicy(globalState.competition),maxJuz:derivedLegacyMaxJuz(categoryScopeOf(category))});};
  ...
  const runFairnessCourt=()=>{const participantIds=new Set(globalState.participants.filter(x=>x.competitionId===globalState.competition.id).map(x=>x.id));const scopedResults=globalState.results.filter(x=>x.competitionId===globalState.competition.id);const contrib=globalState.judgeSubmissions.filter(x=>x.participantId&&participantIds.has(x.participantId)).map(x=>({participantId:x.participantId!,judgeId:x.judgeId,score:x.totalScore}));const rec=runFairnessConstitutionalCourt({competitionId:globalState.competition.id,results:scopedResults,judgeContributions:contrib,allowDropExtreme:true,allowedTieBreakAlternative:false});void hashCanonical({competitionId:rec.competitionId,results:scopedResults.map(x=>({id:x.id,participantId:x.participantId,finalScore:x.finalScore}))}).then(h=>{globalState.fairnessCourtRecords=globalState.fairnessCourtRecords.map(x=>x.id===rec.id?{...x,resultSetHash:h}:x);notify()});globalState.fairnessCourtRecords=[rec,...globalState.fairnessCourtRecords];auditTrustAction('FAIRNESS_CONSTITUTIONAL_REVIEW_RUN','Competition',globalState.competition.id,'تشغيل محكمة الحساسية للعدالة دون تغيير النتائج','Ran non-official fairness sensitivity court without mutating results');notify();return rec;};
  ...
  const routeParticipantByReading=(participantId:string,input?:{requireAudio?:boolean;requireAi?:boolean})=>{const p=globalState.participants.find(x=>x.id===participantId&&x.competitionId===globalState.competition.id);if(!p)return null;const committees=globalState.committees.filter(c=>c.competitionId===globalState.competition.id);const participants=globalState.participants.filter(x=>x.competitionId===globalState.competition.id);const judges=globalState.judges.filter(j=>!j.competitionId||j.competitionId===globalState.competition.id);const counts=new Map(committees.map(c=>[c.id,participants.filter(x=>x.status==='in_queue'&&x.assignedCommitteeId===c.id).length]));const audioReady=new Set(committees.filter(c=>c.audioInputOk).map(c=>c.id));const aiReady=new Set(committees.map(c=>c.id));const rec=multiRiwayahSmartRoute({competitionId:globalState.competition.id,participant:p,committees,judges,queueCounts:counts,audioReadyCommitteeIds:audioReady,aiCompatibleCommitteeIds:aiReady,requireAudio:input?.requireAudio,requireAi:input?.requireAi});globalState.smartRoutingDecisions=[rec,...globalState.smartRoutingDecisions];if(rec.selectedCommitteeId){globalState.participants=globalState.participants.map(x=>x.id===p.id&&x.competitionId===p.competitionId?{...x,assignedCommitteeId:rec.selectedCommitteeId}:x);const routed=globalState.participants.find(x=>x.id===p.id&&x.competitionId===p.competitionId);if(routed)syncParticipantLifecycle(routed)}auditTrustAction('MULTI_RIWAYAH_SMART_ROUTING','Participant',p.id,rec.explanation,rec.explanation);notify();return rec;};
  ...
  return {
    ...
    lastJourneyAccessFailure, lastJourneyAccessNotice,

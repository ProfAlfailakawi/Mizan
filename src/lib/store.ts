...
export function useAppStore() {
  ...
  const applyAuthenticatedIdentity = (identity:{id:string;email:string;name:string;role:Role;organizationId:string;competitionId?:string;mfaEnabled?:boolean;identityAssurance?:User['identityAssurance']}) => {
    globalState.currentUser={id:identity.id,email:identity.email,name:identity.name,nameArabic:identity.name,role:identity.role,organizationId:identity.organizationId,competitionId:identity.competitionId,mfaEnabled:!!identity.mfaEnabled,accountStatus:'active',lastAuthenticatedAt:new Date().toISOString(),identityAssurance:identity.identityAssurance||'firebase'};
  ...
  const registerQuranSourceManifest=async(input:Omit<QuranSourceManifestRecord,'id'|'organizationId'|'status'|'createdAt'>,verses?:QuranVerseRecord[],ayahCountBySurah?:Record<number,number>)=>{
    if(!(globalState.currentUser.role==='scientific_admin'||globalState.currentUser.role==='super_admin'))return null;
  ...
  const reviewQuranSource=async(id:string,decision:'approve'|'reject'='approve',comments='')=>{
    if(!(globalState.currentUser.role==='scientific_admin'||globalState.currentUser.role==='super_admin'))return {ok:false,reason:'not_authorized'};
  ...
  const setScientificReviewersRequired=(count:1|2)=>{
    if(!(globalState.currentUser.role==='scientific_admin'||globalState.currentUser.role==='super_admin'))return false;
  ...
  const certifyQuranSource=async(id:string)=>{
    if(!(globalState.currentUser.role==='scientific_admin'||globalState.currentUser.role==='super_admin'))return false;const i=globalState.quranSourceManifests.findIndex(x=>x.id===id);if(i<0)return false;const x=globalState.quranSourceManifests[i];
  ...
  const registerAiValidation=(input:Omit<AICapabilityValidationRecord,'id'|'organizationId'|'status'|'approvedBy'|'updatedAt'>)=>{
    if(!(globalState.currentUser.role==='scientific_admin'||globalState.currentUser.role==='super_admin'))return null; const rec:AICapabilityValidationRecord={id:newId('aival'),organizationId:globalState.organization.id,status:'validated',certificationState:'PENDING_VALIDATION',validationStage:'RESEARCH',approvedBy:[],updatedAt:new Date().toISOString(),...input}; globalState.aiCapabilityValidations=[rec,...globalState.aiCapabilityValidations];notify();return rec;
  };
  const approveAiCapability=(id:string)=>{
    if(!(globalState.currentUser.role==='scientific_admin'||globalState.currentUser.role==='super_admin'))return false;const i=globalState.aiCapabilityValidations.findIndex(x=>x.id===id);if(i<0)return false;const r=globalState.aiCapabilityValidations[i];const approvedBy=[...new Set([...r.approvedBy,globalState.currentUser.id])];const candidate={...r,approvedBy,approvalVersion:r.approvalVersion||`AI-SG-${new Date().getFullYear()}-${id.slice(-6)}`};
  ...
  const createIdentityInvitation=async(input:{email:string;displayName:string;requestedRole:Role;competitionId?:string;committeeId?:string;reason:string})=>{
    ...
    const dual=roleGrantRequiresDualApproval(input.requestedRole);const expiresAt=new Date(Date.now()+24*60*60*1000).toISOString();let activationToken:string|undefined,activationTokenHash:string|undefined;if(!dual){activationToken=randomInviteToken();activationTokenHash=await invitationTokenHash(activationToken)}
  ...
  const activateIdentityInvitation=async(id:string,token:string,firebaseUid:string,email:string)=>{
    const i=globalState.identityInvitations.findIndex(x=>x.id===id);if(i<0)return {ok:false,reason:'NOT_FOUND'} as const;const inv=globalState.identityInvitations[i];if(inv.status!=='READY'||Date.parse(inv.expiresAt)<=Date.now())return {ok:false,reason:'INVITATION_NOT_USABLE'} as const;if(normalizedIdentityEmail(email)!==inv.email)return {ok:false,reason:'EMAIL_MISMATCH'} as const;if(!inv.activationTokenHash||await invitationTokenHash(token)!==inv.activationTokenHash)return {ok:false,reason:'INVALID_ACTIVATION_TOKEN'} as const;
  ...
  const createAppealCapsule=async(participantId:string,appealId?:string)=>{const evidence=[...globalState.fairDrawProofs.filter(x=>globalState.activeSession.participant?.id===participantId).map(x=>({kind:'fairdraw_proof',ref:x.id})),...globalState.questionRevealGates.filter(x=>x.participantId===participantId).map(x=>({kind:'question_reveal',ref:x.id})),...globalState.sessionCheckpoints.filter(x=>x.participantId===participantId).map(x=>({kind:'continuity_checkpoint',ref:x.id})),...globalState.integrityEnvelopes.filter(x=>x.participantId===participantId).map(x=>({kind:'integrity_envelope',ref:x.id})),...globalState.audioRecordings.filter(x=>x.participantId===participantId).map(x=>({kind:'raw_audio',ref:x.id,private:true}))];const rec=await buildAppealCapsule({competitionId:globalState.competition.id,participantId,appealId,createdBy:globalState.currentUser.id,evidence});globalState.appealCapsules=[rec,...globalState.appealCapsules];auditTrustAction('APPEAL_CAPSULE_CREATED','Participant',participantId,'إنشاء كبسولة استئناف محدودة البيانات','Created privacy-scoped appeal capsule');notify();return rec;};

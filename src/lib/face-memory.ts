...
export interface FaceAttempt {
  ...
  reach?: number;
  /**
   * أيُّ الأحكام جرت فعلًا في هذه التلاوة: `words` حكمُ الكلمات (سقط/أُبدل)، و`teacher` مراجعةُ
   * «المعلّم» (حركة/تجويد/حرف). فغيابُ الملاحظة لا يُعدّ «قرأها صحيحة» إلا إذا جرى حكمُها.
   */
  judged?: { words?: boolean; teacher?: boolean };
}

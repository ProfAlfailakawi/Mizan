...
import { Award, Radio, Wifi, WifiOff, Search, LayoutDashboard, CircleHelp, LogOut } from 'lucide-react';
 ...
 return <header className="sticky top-0 z-30 border-b border-[#DFDED7]/90 bg-[#F7F5EF]/92 backdrop-blur-md">
  <div className="max-w-[1500px] mx-auto h-16 px-4 sm:px-6 flex items-center justify-between gap-3">
    <div className="flex items-center gap-3 min-w-0">
      ...
      <div className="min-w-0 hidden sm:block"><div className="flex items-center gap-2"><span className={`w-1.5 h-1.5 rounded-full ${competition.status==='live'?'bg-[#2F6555]':'bg-[#9B7542]'}`}/><span className="text-[10px] text-[#636864] truncate max-w-[min(52vw,520px)]" title={language==='ar'?competition.nameArabic:competition.name}>{language==='ar'?competition.nameArabic:competition.name}</span></div></div>
    </div>

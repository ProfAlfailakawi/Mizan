...
export const KioskMode: React.FC<{onClose?:()=>void}> = ({onClose}) => {
 const {language,checkInParticipant,committees}=useAppStore(); const ar=language==='ar';
 const [code,setCode]=useState(''); const [done,setDone]=useState<Participant|null>(null); const [error,setError]=useState(false);
 ...
 const submit=(raw=code)=>{const normalized=parseMizanPassPayload(raw);const p=checkInParticipant(normalized,'kiosk_qr');if(p){setDone(p);setError(false);setCode('');stopCamera()}else setError(true)};
 const startCamera=async()=>{
  ...
  try{
   ...
   const tick=async()=>{if(!streamRef.current||!videoRef.current)return;try{const hits=await detector.detect(videoRef.current);const raw=hits?.[0]?.rawValue;if(raw){submit(String(raw));return;}}catch{/* keep manual path available */}scanTimer.current=window.setTimeout(tick,220)};void tick();

...
export const KioskMode: React.FC<{onClose?:()=>void}> = ({onClose}) => {
 ...
 const submit=async(raw=code)=>{const checked=await verifyOfflineJourneyPass(raw);if(!checked.valid){console.warn('MIZAN kiosk pass rejected',checked.reason);setError(true);setErrorReason(checked.reason);return;}const p=checkInParticipant(checked.payload.participantToken,'kiosk_qr');if(p){setDone(p);setError(false);setErrorReason('');setCode('');stopCamera()}else{setError(true);setErrorReason('PARTICIPANT_NOT_ELIGIBLE')}if(!p)console.warn('MIZAN kiosk check-in refused','PARTICIPANT_NOT_ELIGIBLE')};

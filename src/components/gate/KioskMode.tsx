 ...
 return <div className="fixed inset-0 z-50 mizan-venue text-white p-5 sm:p-8 flex flex-col">
   ...
   <div className="my-auto max-w-lg w-full mx-auto">
    {!done?<div className="text-center">
      ...
      <div className="mt-5 max-w-sm mx-auto"><div className="flex gap-2"><div className="relative flex-1"><Keyboard className="w-4 h-4 absolute start-3 top-1/2 -translate-y-1/2 mizan-venue-muted"/><input value={code} onChange={e=>{setCode(e.target.value);setError(false)}} onKeyDown={e=>{if(e.key==='Enter')void submit()}} placeholder={ar?'الكود / قارئ USB':'Code / USB scanner'} className="w-full rounded-xl border border-white/15 bg-white/[.06] ps-10 pe-3 py-3 text-sm text-white placeholder:mizan-venue-faint outline-none focus:border-[#8faf9f]"/></div><Button variant="secondary" disabled={!code.trim()} onClick={()=>void submit()} icon={<ScanLine className="w-4 h-4"/>}>{ar?'دخول':'Go'}</Button></div>{error&&<div className="text-xs text-[#e3afa7] mt-2">{ar?`تعذر التحقق من الاعتماد (${errorReason}). مكتب الاستثناء يساعدك.`:`Credential verification failed (${errorReason}). Use the exception desk.`}</div>}</div>
     </div>
    :<div className="rounded-[32px] border border-white/12 bg-white/[.055] p-8 text-center"><span className="w-16 h-16 rounded-full bg-[#dbe7df] text-[#214C40] grid place-items-center mx-auto"><Check className="w-8 h-8"/></span><div className="mizan-kicker !mizan-venue-muted mt-6">{done.code}</div><h1 className="text-2xl sm:text-3xl font-black mt-2">{ar?`أهلًا ${done.fullNameArabic}`:`Welcome ${done.fullName}`}</h1><div className="mt-5 rounded-[26px] bg-[#f8f6ef] text-[#17352c] p-2 overflow-hidden"><TearOffQueueTicket number={done.originalQueueNumber||done.queueNumber||1} committee={committee?.code} ar={ar} compact/></div><div className="mt-3 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-black/10 p-4"><div className="text-[10px] mizan-venue-muted">{ar?'أمامك':'Ahead'}</div><div className="text-3xl font-black mt-1">{Math.max(0,(done.queueNumber||1)-1)}</div></div><div className="rounded-2xl bg-black/10 p-4"><div className="text-[10px] mizan-venue-muted">{ar?'اللجنة':'Committee'}</div><div className="text-lg font-black mt-2">{committee?.code||'—'}</div></div></div></div>}
   </div>

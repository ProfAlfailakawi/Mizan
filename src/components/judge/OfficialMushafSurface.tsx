...
export const OfficialMushafSurface:React.FC<{question:MushafSurfaceQuestion;ar:boolean;tracking?:QuranAlignmentResult|null}>=({question:q,ar,tracking})=>{
 ...
 /*
  * موضع الكلمة الجارية على الصفحة المطبوعة.
  *
  * ملفات التخطيط المتاحة لا تحمل إحداثيات، لكنها تحمل رقم سطر كل كلمة. فحين لا يوجد صندوق
  * تنتقل العدسة إلى **سطر الكلمة** وحده — تتبّعٌ يمشي مع التلاوة سطرًا سطرًا بدل أن يغطّي
  * المقطع كله. وعدد أسطر الصفحة يُؤخذ من الملف لا من رقم ثابت: الصفحتان الافتتاحيتان ثمانية
  * أسطر لا خمسة عشر، وتثبيتُ العدد كان يزيح العدسة عنهما.
  */
...
export const PassageAudio:React.FC<{reading:string;ayat:{surah:number;ayah:number;text?:string}[];ar:boolean;onActive?:(state:{ayah:number;word:number}|null)=>void}>=({reading,ayat,ar,onActive})=>{
 ...
 /* المرجع الافتتاحي: تُتلى آية السؤال الأولى فقط ثم يسكت — لا يُكمِل بقية المقطع تلقائيًا.
    (كان يتقدّم آية آية فيقرأ الوجه كله؛ وهذا خطأ: المطلوب أول آية ثم صمت ليبدأ المتسابق.) */
 ...
 return <div className="px-4 sm:px-5 py-3 border-t border-[#e5e1d7] bg-[#f7f4ec]">
  <div className="flex items-center justify-between gap-3">
   <div className="flex items-center gap-3 min-w-0">
    ...
    <div className="min-w-0"><div className="text-[10px] font-black truncate">{ar?'تلاوة أول آية للسؤال':'First ayah of the question'}</div>

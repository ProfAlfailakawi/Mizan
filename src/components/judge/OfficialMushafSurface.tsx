  ...
  {audioSpot?.bbox
   ...
   : audioSpot?.line
     ...
     : audioFocus
       ...
       : null}{word&&<WordVectorLens bbox={word}/>}</div><figcaption className="mt-2 text-center text-[9px] font-black text-[#606661]">{ar?'الصفحة':'Page'} {locus.page}{!focus.lineCount?<span className="ms-2 font-normal text-[#696f6b]">{ar?'هندسة الأسطر غير متاحة — بلا تخمين':'line geometry unavailable — no guess'}</span>:null}</figcaption></figure>}

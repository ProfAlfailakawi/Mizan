...
export const LanguageSwitcher: React.FC<{ compact?: boolean }> = ({ compact = false }) => {
  ...
  return <label className="inline-flex items-center gap-2 bg-[#F5F2EB] px-2.5 py-1.5 rounded-xl border border-[#EAE4DC] text-[#625f59]">
    <Globe className="w-3.5 h-3.5 text-[#8b877f]" aria-hidden="true"/>
    <span className="sr-only">Language</span>
    <select value={language} onChange={e=>setLanguage(e.target.value as SupportedLanguage)} className={`bg-transparent outline-none text-xs font-bold ${compact?'max-w-20':'max-w-36'}`} aria-label="Language">

...
/** User-facing roles only. scientific_admin is retired and therefore intentionally absent. */
...
export const RoleSwitcher: React.FC = () => {
  ...
  return <div className="relative">
    <button onClick={()=>setOpen(v=>!v)} aria-expanded={open} aria-haspopup="menu" className="flex items-center gap-2 rounded-xl px-2.5 py-2 hover:bg-[#efede7] transition" aria-label={language==='ar'?'تبديل الدور التجريبي':'Switch demo role'}>
      ...
      <span className="hidden sm:block text-start leading-tight"><span className="block text-xs font-bold text-[#303733] max-w-32 truncate">{language==='ar'?(currentUser.nameArabic||currentUser.name):currentUser.name}</span><span className="block text-[10px] text-[#666a67]">{language==='ar'?meta.ar:meta.en}</span></span>

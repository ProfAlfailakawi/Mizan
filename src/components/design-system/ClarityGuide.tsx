...
import { CircleHelp, X, ShieldCheck, ScanSearch, Ticket, Gavel, RadioTower, UserRound } from 'lucide-react';
...
const ROLE_GUIDE:Partial<Record<Role,{titleAr:string;titleEn:string;steps:Step[]}>>={
 ...
 head_judge:{titleAr:'ما يحتاجك فقط',titleEn:'Only what needs you',steps:[
  {titleAr:'راجع الاختلافات',titleEn:'Review disagreements',noteAr:'تصل لك الحالات التي تحتاج قرارًا بشريًا أو اعتمادًا متخصصًا.',noteEn:'Only cases needing human or specialist review reach you.'},
 ...
 org_admin:{titleAr:'إدارة الجهة بلا تعقيد',titleEn:'Run your organization',steps:[
  {titleAr:'أنشئ فريقك',titleEn:'Provision your team',noteAr:'بعد اعتماد مالك ميزان لك، تضيف أدوار الجهة المسموحة مباشرة دون انتظار موافقة شخص ثانٍ.',noteEn:'Once the platform owner delegates your authority, you provision allowed tenant roles directly.'},
  {titleAr:'كل شخص بحسابه',titleEn:'One person, one account',noteAr:'الحسابات معزولة داخل جهتك، وكل إضافة أو حذف أو تعليق له أثر تدقيقي.',noteEn:'Accounts remain tenant-scoped and every provision, removal, or suspension is audited.'},
 ]},
...
const TERMS:Step[]=[
 ...
 {titleAr:'مصدر معتمد',titleEn:'Certified source',noteAr:'حزمة قرآن محددة النسخة والبصمة وافق عليها أصحاب الصلاحية المعتمدون.',noteEn:'An exact versioned Quran package approved by authorized reviewers.'},
...
export const ClarityGuide:React.FC<Props>=({open,onClose,role,ar})=>{
 ...
 const roleIcon=role==='judge'||role==='head_judge'?Gavel:role==='ops_manager'?RadioTower:role==='participant'?UserRound:ShieldCheck;
 const Icon=roleIcon;
 return createPortal(
  <div className="fixed inset-0 z-[230] bg-[#171b18]/35 backdrop-blur-[2px]" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}>
   <aside ref={panelRef as any} className={`absolute top-0 bottom-0 w-full max-w-md bg-[#FFFEFB] shadow-2xl p-5 sm:p-6 pb-[calc(1.5rem+env(safe-area-inset-bottom))] overflow-y-auto overscroll-contain ${ar?'left-0 border-r':'right-0 border-l'} border-[#dedcd5]`} role="dialog" aria-modal="true" aria-label={ar?'دليل مبسط':'Plain-language guide'}>
    <div className="flex items-start justify-between gap-4"><div className="flex gap-3"><span className="w-11 h-11 rounded-2xl bg-[#E7EEE9] text-[#214C40] grid place-items-center"><Icon className="w-5 h-5"/></span><div><div className="text-[10px] font-black tracking-[.15em] text-[#68716b]">{ar?'شرح مبسط':'PLAIN LANGUAGE'}</div><h2 className="text-xl font-black mt-1">{ar?guide.titleAr:guide.titleEn}</h2></div></div><Button shape="square" variant="ghost" onClick={onClose} aria-label={ar?'إغلاق':'Close'}><X className="w-4 h-4"/></Button></div>

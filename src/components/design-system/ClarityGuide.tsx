...
export const ClarityGuide:React.FC<Props>=({open,onClose,role,ar})=>{
 ...
 const Icon=roleIcon;
 return <div className="fixed inset-0 z-[200] bg-[#171b18]/35 backdrop-blur-[2px]" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}>
  <aside ref={panelRef as any} className={`absolute top-0 bottom-0 w-full max-w-md bg-[#FFFEFB] shadow-2xl p-5 sm:p-6 pb-[calc(1.5rem+env(safe-area-inset-bottom))] overflow-y-auto ${ar?'left-0 border-r':'right-0 border-l'} border-[#dedcd5]`} role="dialog" aria-modal="true" aria-label={ar?'دليل مبسط':'Plain-language guide'}>

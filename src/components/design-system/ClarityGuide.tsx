...
export const ClarityGuide:React.FC<Props>=({open,onClose,role,ar})=>{
 ...
 return <div className="fixed inset-0 z-[90] bg-[#171b18]/35 backdrop-blur-[2px]" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}>
  <aside className={`absolute top-0 bottom-0 w-full max-w-md bg-[#FFFEFB] shadow-2xl p-5 sm:p-6 overflow-y-auto ${ar?'left-0 border-r':'right-0 border-l'} border-[#dedcd5]`} role="dialog" aria-modal="true" aria-label={ar?'دليل مبسط':'Plain-language guide'}>
   ...
   <div className="mt-6 divide-y divide-[#e6e4dd]">{guide.steps.map((x,i)=><div key={`${x.titleEn}-${i}`} className="py-4 grid grid-cols-[30px_1fr] gap-3"><span className="w-7 h-7 rounded-lg bg-[#f0eee8] grid place-items-center text-[11px] font-black">{i+1}</span><div><div className="text-sm font-black">{ar?x.titleAr:x.titleEn}</div><div className="text-xs leading-6 text-[#707873] mt-1">{ar?x.noteAr:x.noteEn}</div></div></div>)}</div>
   <div className="mt-7 pt-5 border-t border-[#dedcd5]"><div className="flex items-center gap-2"><CircleHelp className="w-4 h-4 text-[#53615a]"/><div className="text-xs font-black">{ar?'مصطلحات قد تراها':'Terms you may see'}</div></div><div className="mt-3 space-y-2">{TERMS.map(x=><details key={x.titleEn} className="rounded-xl bg-[#f5f3ed] px-3 py-2.5"><summary className="cursor-pointer text-xs font-black list-none flex items-center justify-between gap-2"><span>{ar?x.titleAr:x.titleEn}</span><ScanSearch className="w-3.5 h-3.5 text-[#78807b]"/></summary><p className="text-[11px] leading-5 text-[#707873] mt-2">{ar?x.noteAr:x.noteEn}</p></details>)}</div></div>

...
const localizedBadgeChild=(child:React.ReactNode)=>{if(typeof child!=='string')return child;const ar=typeof document!=='undefined'&&(document.documentElement.lang||'').toLowerCase().startsWith('ar');return uiToken(child,ar)};
export const Badge: React.FC<BadgeProps> = ({children,variant='emerald',size='sm',className='',dot=true}) => {
 ...
 return <span className={`inline-flex items-center rounded-full border font-semibold whitespace-nowrap ${size==='sm'?'text-[11px] px-2.5 py-1 gap-1.5':'text-xs px-3 py-1.5 gap-2'} ${v[variant]} ${className}`}>{dot&&<span className={`w-1.5 h-1.5 rounded-full ${d[variant]}`}/>}<span>{localizedBadgeChild(children)}</span></span>
}

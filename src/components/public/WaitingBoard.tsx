...
export const WaitingBoard:React.FC<{onClose?:()=>void}>=({onClose})=>{
 ...
 return <div ref={venueRef} role={onClose?"dialog":undefined} aria-modal={onClose?true:undefined} aria-label={ar?'شاشة الانتظار':'Waiting display'} className="fixed inset-0 z-50 mizan-venue-2 text-white font-arabic overflow-auto">
   <div className="min-h-screen p-5 sm:p-8 lg:p-10 flex flex-col">
    ...
    <footer className="grid sm:grid-cols-3 gap-2"><Stat icon={UsersRound} n={waiting.length} t={ar?'ينتظرون':'Waiting'}/><Stat icon={Clock3} n={committees.filter(c=>c.status==='testing').length} t={ar?'لجان تعمل':'Active panels'}/><div className="rounded-2xl border border-white/8 px-4 py-3 text-[11px] mizan-venue-muted flex items-center">{ar?'تُعرض الأكواد فقط احترامًا للخصوصية. تعمل على أي تلفاز أو شاشة متصلة بمتصفح.':'Codes only by default for privacy. Runs on any TV or display with a browser.'}</div></footer>
   </div>
 </div>
}

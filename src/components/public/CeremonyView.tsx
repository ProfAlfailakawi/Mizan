...
export const CeremonyView: React.FC<{onClose?:()=>void}> = ({onClose}) => {
 ...
 const canApprove=!!reveal?.requiredRoleGroups.flat().includes(currentUser.role)&&!approved&&reveal.status!=='executed';
 const canExecute=reveal?.status==='ready'&&['broadcast_operator','comp_admin','org_admin'].includes(currentUser.role);
 const request=async()=>{setBusy(true);try{await s.requestCeremonyReveal()}finally{setBusy(false)}};
 ...
 return <div className="fixed inset-0 z-50 bg-[#101a16] text-white overflow-hidden flex flex-col p-5 sm:p-8">
   ...
   <main className="flex-1 grid place-items-center px-3"><div className="w-full max-w-5xl text-center">
    {!authorized?<div className="max-w-xl mx-auto">
      ...
      <div className="text-[10px] font-black tracking-[.22em] text-[#b9cfc4] mt-6">QUORUM SEAL</div>
      <h1 className="text-3xl sm:text-5xl font-black mt-4 tracking-tight">{ar?'النتائج محمية':'Results remain sealed'}</h1>
      <p className="text-sm text-white/45 mt-4">{ar?'لا يستطيع شخص واحد كشف النتائج. يلزم اعتمادان مستقلان ثم تنفيذ صريح.':'No single person can reveal results. Two independent authorities must approve, then reveal is explicitly executed.'}</p>
      <div className="mt-7 grid grid-cols-2 gap-2 text-start">
        <Approval ok={!!reveal?.approvals.some(a=>a.actorRole==='head_judge')} label={ar?'التحكيم':'Judging'}/>
        <Approval ok={!!reveal?.approvals.some(a=>['comp_admin','org_admin'].includes(a.actorRole))} label={ar?'الإدارة':'Authority'}/>
      </div>
      <div className="mt-5 flex flex-wrap justify-center gap-2">
        {!reveal&&<Button loading={busy} onClick={request} icon={<KeyRound className="w-4 h-4"/>}>{ar?'طلب الكشف':'Request reveal'}</Button>}
        ...
        {canExecute&&<Button onClick={()=>reveal&&s.executeQuorumAction(reveal.id)} icon={<ShieldCheck className="w-4 h-4"/>}>{ar?'فتح الحفل':'Open reveal'}</Button>}
        {reveal&&<Badge variant={reveal.status==='ready'?'amber':'neutral'}>{reveal.status}</Badge>}
      </div>
    </div>

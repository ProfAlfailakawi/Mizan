...
export const RegistrationFlow: React.FC<{onSuccess?:(participant:Participant)=>void;localPreview?:boolean}> = ({onSuccess,localPreview=false}) => {
 ...
 const consentClauses={terms:'أوافق على شروط المشاركة',privacy:'أوافق على سياسة الخصوصية',assistiveAi:'أوافق اختياريًا على المعالجة التقنية المساندة عند تفعيلها. رفضها لا يمنع إرسال الطلب.'};
    ...
    try{
      ...
      if(!response.ok){
        if(response.status===429||body.code==='RATE_LIMITED'){
          ...
          const waitSec=retryAfter?Math.max(1,parseInt(retryAfter,10)):10;

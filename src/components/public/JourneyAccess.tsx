...
export const JourneyAccess:React.FC<{audience:Audience}>=({audience})=>{
  ...
  return <div className="min-h-screen bg-[#FAF8F2] text-[#171B18]" dir={ar?'rtl':'ltr'}>
    ...
    <main className="max-w-4xl mx-auto px-4 py-8 sm:py-12">
      ...
      <section className="mizan-surface p-5 sm:p-6"><div className="text-sm font-black">{ar?'مسار الرحلة':'Journey timeline'}</div><div className="mt-5 overflow-x-auto pb-2"><div className="min-w-[720px] flex items-start">{Array.from({length:9},(_,i)=><React.Fragment key={i}><div className="w-20 shrink-0 text-center"><div className={`mx-auto w-9 h-9 rounded-xl grid place-items-center ${i<idx?'bg-[#214C40] text-white':i===idx?'bg-[#E8CB93] text-[#183a31]':'bg-[#f0eee8] text-[#8a8f8b]'}`}>{i<idx?<BadgeCheck className="w-4 h-4"/>:<span className="text-xs font-black">{i+1}</span>}</div><div className="mt-2 text-[9px] leading-4 font-bold">{stepLabel(i,ar)}</div></div>{i<8&&<div className={`h-px flex-1 mt-[18px] ${i<idx?'bg-[#214C40]':'bg-[#ddd9d0]'}`}/>}</React.Fragment>)}</div></div></section>

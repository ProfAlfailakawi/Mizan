  ...
  return <div className="min-h-screen bg-[#FAF8F2] text-[#171B18]" dir={ar ? 'rtl' : 'ltr'}>
    ...
    <main className="max-w-4xl mx-auto px-4 py-8 sm:py-12">
      {!journey ? <section className="mizan-surface max-w-xl mx-auto p-6 sm:p-8 text-center">
        <div className="mx-auto w-14 h-14 rounded-2xl bg-[#E7EEE9] text-[#214C40] grid place-items-center">{audience === 'guardian' ? <UsersRound className="w-6 h-6" /> : <UserRound className="w-6 h-6" />}</div>
        ...
        {canPrepare && <section className="mizan-surface p-5 sm:p-6"><div className="mb-4"><div className="mizan-kicker">{ar ? 'التحضير للاختبار' : 'TEST PREPARATION'}</div><h2 className="mt-1 text-lg font-black">{ar ? 'هيّئ نطاقك ونفَسك وجرّب بروفة خاصة' : 'Settle your range, breathing, and private rehearsal'}</h2><p className="mt-1 text-[11px] leading-6 text-[#646965]">{ar ? 'هذه التهيئة لك وحدك؛ لا تُرسل ملاحظاتها إلى اللجنة ولا تمس درجتك.' : 'This preparation is private; nothing is sent to the panel or affects your score.'}</p></div><WarmupSanctuary ar={ar} scopeText={ar ? journey.preparation?.scopeTextArabic || undefined : journey.preparation?.scopeTextEnglish || undefined} spreadAcrossZones={journey.preparation?.spreadAcrossZones} questionCount={journey.preparation?.questionCount} minutesPerQuestion={journey.preparation?.minutesPerQuestion}/></section>}

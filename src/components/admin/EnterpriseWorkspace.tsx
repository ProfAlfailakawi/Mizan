...
const Integrations=({s,ar,pending}:{s:ReturnType<typeof useAppStore>;ar:boolean;pending:number})=>{
 ...
 return <div className="space-y-4"><div className="grid xl:grid-cols-[1fr_.7fr] gap-4"><div className="mizan-surface p-5"><div className="flex items-center justify-between"><h2 className="font-extrabold">{ar?'القنوات':'Channels'}</h2><Badge variant={pending?'amber':'emerald'}>{pending?`${pending} ${ar?'تعثر':'failed'}`:(ar?'هادئ':'Quiet')}</Badge></div><div className="grid sm:grid-cols-2 gap-2 mt-4">{kinds.map(k=>{const cfg=s.integrations.find(i=>i.kind===k);const live=cfg?.status==='configured';return <div key={k} className="rounded-xl border border-[#e3e1da] p-3"><div className="flex items-center justify-between"><span className="text-xs font-extrabold uppercase">{k}</span><span className={`w-2 h-2 rounded-full ${live?'bg-[#2F6555]':'bg-[#c9c5bb]'}`}/></div><div className="text-[10px] text-[#656b66] mt-2">{live?(ar?'متصل بمزود فعلي':'Provider live'):(ar?'يتطلب إعداد مزود خارجي':'External setup required')}</div></div>})}</div></div>

...
export const EnterpriseWorkspace:React.FC=()=>{
 ...
 // إدارة الحسابات والصلاحيات تخص مدير الجهة (ومالك المنصة) وحدهما؛ مدير المسابقة يشغّل
 // مسابقته ولا يمنح صلاحيات — فلا يوجد مدخلان لإنشاء الحسابات.
 const canManageIdentities=['org_admin','super_admin'].includes(s.currentUser.role);
 const ownerOnly:Section[]=['shadow','trust','beyond'];
 const allSections:[Section,any,string][]=[['integrations',Cable,ar?'القنوات':'Channels'],['operations',RadioTower,ar?'البنية الميدانية':'Field'],['international',Plane,ar?'الدولي':'International'],['shadow',Radar,ar?'الظل':'Shadow'],['governance',ShieldCheck,ar?'الحوكمة':'Governance'],['trust',Fingerprint,ar?'الثقة':'Trust 8'],['beyond',WandSparkles,ar?'ما بعد':'Beyond']];
 ...
 return <div className="space-y-4">
  ...
  {activeSection==='governance'&&<div className="space-y-4">{canManageIdentities&&<Suspense fallback={<PanelFallback/>}><IdentityGovernance/></Suspense>}<Suspense fallback={<PanelFallback/>}><ReadinessLab onNavigate={t=>setSection(t==='field'?'operations':'integrations')}/></Suspense><Governance s={s} ar={ar}/></div>}
...
const Integrations=({s,ar,pending}:{s:ReturnType<typeof useAppStore>;ar:boolean;pending:number})=>{
 ...
 return <div className="space-y-4"><div className="grid xl:grid-cols-[1fr_.7fr] gap-4"><div className="mizan-surface p-5"><div className="flex items-center justify-between"><h2 className="font-extrabold">{ar?'القنوات':'Channels'}</h2><Badge variant={pending?'amber':'emerald'}>{pending?`${pending} ${ar?'تعثر':'failed'}`:(ar?'هادئ':'Quiet')}</Badge></div><div className="grid sm:grid-cols-2 gap-2 mt-4">{kinds.map(k=>{const cfg=s.integrations.find(i=>i.kind===k);const live=cfg?.status==='configured';return <div key={k} className="rounded-xl border border-[#e3e1da] p-3"><div className="flex items-center justify-between"><span className={`text-xs font-extrabold ${ar?'':'uppercase'}`}>{ar?channelLabel(k,true):k}</span><span className={`w-2 h-2 rounded-full ${live?'bg-[#2F6555]':'bg-[#c9c5bb]'}`}/></div><div className="text-[10px] text-[#656b66] mt-2">{live?(ar?'مفعّلة وتعمل':'Provider live'):(ar?'يفعّلها فريق ميزان عند التشغيل':'Activated by the MIZAN team')}</div></div>})}</div></div>

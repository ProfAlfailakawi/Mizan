...
export const EnterpriseWorkspace:React.FC=()=>{
 ...
 const allSections:[Section,any,string][]=[['integrations',Cable,ar?'القنوات':'Channels'],['operations',RadioTower,ar?'البنية الميدانية':'Field'],['international',Plane,ar?'الجهات':'Entities'],['shadow',Radar,ar?'الظل':'Shadow'],['governance',ShieldCheck,ar?'الجاهزية':'Readiness'],['tools',DatabaseBackup,ar?'أدوات الإدارة':'Admin tools'],['trust',Fingerprint,ar?'الثقة':'Trust 8'],['beyond',WandSparkles,ar?'ما بعد':'Beyond']];
 ...
 return <div className="space-y-4">
  <div><div className="mizan-kicker">{ar?'طبقة المؤسسات':'ENTERPRISE LAYER'}</div><h1 className="text-2xl sm:text-3xl font-black mt-1">{ar?'القوة مخفية خلف البساطة':'Enterprise power, quietly contained'}</h1><p className="text-sm text-[#626864] mt-2 max-w-2xl">{ar?'تكاملات والجهات المشاركة واستمرارية وحوكمة، دون تلويث تجربة المستخدم اليومية.':'Integrations, participating entities, continuity and governance stay out of the daily user flow.'}</p></div>
  ...
  {activeSection==='operations'&&<FieldOperations s={s} ar={ar} online={online}/>}
...
const Governance=({s,ar}:{s:ReturnType<typeof useAppStore>;ar:boolean})=>{
 ...
 const clone=async()=>{if(!(await confirm({
  ...
  body:ar?'تُنشأ مسابقة جديدة بالفئات والقواعد ومعايير التحكيم نفسها. ولا يُنسخ المتسابقون ولا النتائج ولا التواريخ؛ تبدأ فارغة كمسودّة.':'A new competition is created with the same categories, rules and rulebook. Participants, results and dates are not copied; it starts empty as a draft.',

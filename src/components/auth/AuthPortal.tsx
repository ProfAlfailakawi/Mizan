...
export const AuthPortal:React.FC=()=>{
 ...
 return <div className="min-h-screen bg-[#F8F5ED] grid place-items-center p-5" dir={ar?'rtl':'ltr'}><div className="w-full max-w-md">
  <div className="text-center"><div className="flex justify-center"><MizanLogo language={language}/></div><div className="mizan-kicker mt-7">{ar?'وصول محمي':'Secure access'}</div><h1 className="text-3xl font-black mt-2 text-[#17352D]">{ar?'تسجيل الدخول':'Sign in'}</h1><p className="text-xs text-[#737a75] mt-2 leading-6">{ar?'تُطبّق هوية المستخدم وصلاحيات المؤسسة قبل السماح بأي إجراء تشغيلي.':'Identity and tenant permissions are enforced before operational access.'}</p></div>
  <div className="mizan-surface p-6 mt-7"><label className="block"><span className="text-[10px] font-black text-[#767d78]">{ar?'البريد الإلكتروني':'Email'}</span><div className="relative mt-2"><Mail className="w-4 h-4 absolute start-3 top-1/2 -translate-y-1/2 text-[#8b918d]"/><input autoComplete="email" value={email} onChange={e=>setEmail(e.target.value)} className="w-full rounded-xl border border-[#ddd] ps-10 pe-3 py-3 text-sm"/></div></label>
  <label className="block mt-4"><span className="text-[10px] font-black text-[#767d78]">{ar?'كلمة المرور':'Password'}</span><div className="relative mt-2"><LockKeyhole className="w-4 h-4 absolute start-3 top-1/2 -translate-y-1/2 text-[#8b918d]"/><input type="password" autoComplete="current-password" value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={e=>e.key==='Enter'&&signIn()} className="w-full rounded-xl border border-[#ddd] ps-10 pe-3 py-3 text-sm"/></div></label>
  ...
  <p className="text-[10px] text-[#858b87] text-center mt-5 leading-5">{ar?'تخضع الأدوار الحساسة للمصادقة متعددة العوامل وسياسات التصعيد المعتمدة لدى المؤسسة.':'MFA and step-up policies are tenant-controlled.'}</p>
 </div></div>
}

 ...
 return <div className="min-h-screen bg-[#F8F5ED] grid place-items-center p-5" dir={ar?'rtl':'ltr'}><div className="w-full max-w-md">
  ...
  <div className="mizan-surface p-6 mt-7">
   {previewLoading?<div className="py-8 text-center text-xs font-bold text-[#636864]">{ar?'جارٍ قراءة الدعوة…':'Reading invitation…'}</div>:<>
    <label className="block"><span className="text-[10px] font-black text-[#646965]">{ar?'البريد الإلكتروني':'Email'}</span><div className="relative mt-2"><Mail className="w-4 h-4 absolute start-3 top-1/2 -translate-y-1/2 text-[#6a6f6c] pointer-events-none"/><input autoComplete="email" value={email} readOnly={activating} onChange={e=>setEmail(e.target.value)} className={`mizan-input mizan-input-icon-start text-sm ${activating?'bg-[#f5f4ef] text-[#4f5551]':''}`}/></div></label>

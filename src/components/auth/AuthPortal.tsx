...
export const AuthPortal:React.FC=()=>{
 ...
 const signIn=async()=>{
  ...
  try{
   ...
   {const serverCode=raw.includes(':')?String(raw.split(':').pop()||'').trim():'';setMessage(ar?(AUTH_AR[code]||(serverCode?serverErrorLabel(serverCode,true):'تعذّر تسجيل الدخول. تحقق من البيانات وحاول مرة أخرى.')):(AUTH_AR[code]||'Sign-in failed'));}
 ...
 const createInvitedAccount=async()=>{
  ...
  try{
   ...
   {const serverCode=raw.includes(':')?String(raw.split(':').pop()||'').trim():'';setMessage(ar?(AUTH_AR[code]||(serverCode?serverErrorLabel(serverCode,true):'تعذر إنشاء الحساب. تحقق من البيانات وحاول مرة أخرى.')):(AUTH_AR[code]||'Could not create account'));}
  }finally{setBusy(false)}
 };

 const finishMfaSignIn=async()=>{if(!mfaResolver)return;const clean=mfaCode.replace(/\s+/g,'');if(!/^\d{6}$/.test(clean)){setMessage(ar?'اكتب رمز Authenticator المكوّن من 6 أرقام.':'Enter the 6-digit Authenticator code.');return;}const hint=mfaResolver.hints.find(h=>h.factorId===TotpMultiFactorGenerator.FACTOR_ID);if(!hint){setMessage(ar?'عامل Authenticator غير موجود لهذا الحساب.':'No Authenticator factor is enrolled on this account.');return;}setBusy(true);setMessage('');try{const assertion=TotpMultiFactorGenerator.assertionForSignIn(hint.uid,clean);const cred=await mfaResolver.resolveSignIn(assertion);setMfaResolver(null);setMfaCode('');if(activationToken)await finishActivation(cred.user);}catch(e:unknown){const code=String((e as {code?:string})?.code||'');setMessage(code==='auth/invalid-verification-code'?(ar?'الرمز غير صحيح أو انتهت مدته. جرّب الرمز الحالي في التطبيق.':'The code is invalid or expired. Use the current code from your app.'):(ar?'تعذر إكمال التحقق بخطوتين. حاول مرة أخرى.':'Could not complete two-step verification.'));}finally{setBusy(false)}};

 const reset=async()=>{if(!email.trim())return setMessage(ar?'أدخل البريد الإلكتروني أولًا':'Enter your email first');setBusy(true);setMessage('');try{const r=await fetch('/api/identity/password-reset/request',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email:email.trim()})});if(!r.ok&&r.status!==202)throw new Error('REQUEST_FAILED');setMessage(ar?'تم إرسال طلب تعيين كلمة مرور جديدة إلى المدير المسؤول. لن يُرسل النظام بريد استعادة تلقائيًا؛ سيصلك رابط آمن أو QR من مدير الجهة بعد اعتماده.':'Your password reset request was sent to the responsible administrator. No automatic recovery email is sent.');}catch{setMessage(ar?'تعذر إرسال الطلب الآن. تحقق من الاتصال وحاول مرة أخرى.':'Could not submit the reset request. Try again.');}finally{setBusy(false)}};

 if(mfaResolver)return <div className="min-h-screen bg-[#F8F5ED] grid place-items-center p-5" dir={ar?'rtl':'ltr'}><div className="w-full max-w-md"><div className="text-center"><div className="flex justify-center"><MizanLogo language={language}/></div><div className="mizan-kicker mt-7">{ar?'تحقق بخطوتين':'TWO-STEP VERIFICATION'}</div><h1 className="text-3xl font-black mt-2 text-[#17352D]">{ar?'رمز Authenticator':'Authenticator code'}</h1><p className="text-xs text-[#636864] mt-2 leading-6">{ar?'افتح Google Authenticator واكتب الرمز الحالي المكوّن من 6 أرقام.':'Open Google Authenticator and enter the current 6-digit code.'}</p></div><div className="mizan-surface p-6 mt-7"><label className="block"><span className="text-[10px] font-black text-[#646965]">{ar?'رمز التحقق':'Verification code'}</span><div className="relative mt-2"><KeyRound className="w-4 h-4 absolute start-3 top-1/2 -translate-y-1/2 text-[#6a6f6c] pointer-events-none"/><input autoFocus inputMode="numeric" autoComplete="one-time-code" maxLength={6} value={mfaCode} onChange={e=>setMfaCode(e.target.value.replace(/\D/g,'').slice(0,6))} onKeyDown={e=>e.key==='Enter'&&void finishMfaSignIn()} className="mizan-input mizan-input-icon-start text-center tracking-[0.3em] font-black" dir="ltr"/></div></label>{message&&<div className="mt-4 rounded-xl bg-[#f3f1eb] px-3 py-2.5 text-xs font-semibold">{message}</div>}<Button className="w-full mt-5" disabled={busy||mfaCode.length!==6} onClick={()=>void finishMfaSignIn()} icon={<ShieldCheck className="w-4 h-4"/>}>{busy?'…':ar?'تحقق وادخل':'Verify & sign in'}</Button><button type="button" onClick={()=>{setMfaResolver(null);setMfaCode('');setMessage('')}} className="w-full min-h-11 mt-3 text-xs font-bold text-[#45675b]">{ar?'العودة لتسجيل الدخول':'Back to sign in'}</button></div></div></div>;
 ...
 return <div className="min-h-screen bg-[#F8F5ED] grid place-items-center p-5" dir={ar?'rtl':'ltr'}><div className="w-full max-w-md">
  ...
  <div className="mizan-surface p-6 mt-7">
   {previewLoading?<div className="py-8 text-center text-xs font-bold text-[#636864]">{ar?'جارٍ قراءة الدعوة…':'Reading invitation…'}</div>:<>
    <label className="block"><span className="text-[10px] font-black text-[#646965]">{ar?'البريد الإلكتروني':'Email'}</span><div className="relative mt-2"><Mail className="w-4 h-4 absolute start-3 top-1/2 -translate-y-1/2 text-[#6a6f6c] pointer-events-none"/><input type="email" dir="ltr" lang="en" autoComplete="email" autoCapitalize="none" autoCorrect="off" spellCheck={false} value={email} readOnly={activating} onChange={e=>setEmail(e.target.value)} className={`mizan-input mizan-input-icon-start text-sm text-left ${activating?'bg-[#f5f4ef] text-[#4f5551]':''}`} style={{unicodeBidi:'plaintext'}}/></div></label>
    ...
    <label className="block mt-4"><span className="text-[10px] font-black text-[#646965]">{activating&&!existingMode?(ar?'اختر كلمة المرور':'Choose password'):(ar?'كلمة المرور':'Password')}</span><div className="relative mt-2"><LockKeyhole className="w-4 h-4 absolute start-3 top-1/2 -translate-y-1/2 text-[#6a6f6c] pointer-events-none"/><input type="password" autoComplete={activating&&!existingMode?'new-password':'current-password'} value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={e=>e.key==='Enter'&&(activating&&!existingMode?createInvitedAccount():signIn())} className="mizan-input mizan-input-icon-start text-sm"/></div></label>
    {activating&&!existingMode&&<label className="block mt-4"><span className="text-[10px] font-black text-[#646965]">{ar?'تأكيد كلمة المرور':'Confirm password'}</span><div className="relative mt-2"><LockKeyhole className="w-4 h-4 absolute start-3 top-1/2 -translate-y-1/2 text-[#6a6f6c] pointer-events-none"/><input type="password" autoComplete="new-password" value={confirmPassword} onChange={e=>setConfirmPassword(e.target.value)} onKeyDown={e=>e.key==='Enter'&&createInvitedAccount()} className="mizan-input mizan-input-icon-start text-sm"/></div></label>}

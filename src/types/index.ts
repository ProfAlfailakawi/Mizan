...
export interface WitnessModeRecord { id:string;competitionId:string;organizationId:string;actionType:'QUESTION_REPLACEMENT'|'FULL_RETEST'|'PASS_REISSUE'|'EMERGENCY_UNSEAL'|'RESULT_OVERRIDE';targetRef:string;reason:string;initiatedBy:string;createdAt:string;expiresAt:string;requiredWitnesses:number;attestations:{uid:string;role:string;attestedAt:string}[];state:'PENDING'|'READY'|'EXPIRED'|'REVOKED';commitmentHash:string; }

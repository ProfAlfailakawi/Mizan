...
export interface Organization {
  ...
  createdAt: string;
  /**
   * عدد المراجعين العلميين المستقلين اللازمين لاعتماد مصدر قرآني. الافتراضي اثنان.
   * تضبطه السلطة العلمية للجهة صراحةً، ويُسجَّل في الأثر: اختيار «واحد» يعني أن مراجعًا
   * واحدًا هو السلطة العلمية كاملةً — قرار معلن لا تجاوز صامت.
   */
  scientificReviewersRequired?: 1 | 2;

  /** النطاق الفرعي تحت النطاق الأساسي (jamiat-x.mizan.app) — للعرض؛ سجل الخادم هو المرجع. */
  subdomain?: string;
  /** نطاقات خاصة توجّهها الجهة إلى هذا النشر. */
...
export interface Category {
  ...
  /**
   * مقدار الموضع من وجه المصحف: وجه كامل أو نصفه أو ثلثه أو ربعه.
   * الوجه يُعرض كاملًا دائمًا على سطح المصحف؛ وهذا الإعداد يحدد كم منه يغطيه الموضع
...
export interface Participant {
  ...
  /**
   * ما وُعد به من انتظار لحظة دخوله الطابور — يُختم عندها لا عند النداء، وإلا قُورن
   * التقديرُ بنفسه. يُصفّى عند ندائه فيُعرف كم كان الوعد صادقًا.
   */
...
export interface JudgingPolicy {
  ...
  /* كل مسابقة تقرر: هل يرى المحكم مجموع الخصم الجاري؟ وهل يُسمح له بالتراجع عن آخر ملاحظة؟
     غياب القيمة يعني السماح (توافق رجعي مع اللوائح القديمة). */
  showRunningScoreToJudge?: boolean;
  allowJudgeUndo?: boolean;
}
...
export interface OperationsPolicy {
  ...
  /* ── توزيع المشاركين على اللجان ───────────────────────────────────────────
   * القرار كان مخبوءًا في الشيفرة: أقلّ اللجان عددًا عند مسح البطاقة، بلا خيار. وحين
   * صارت اللجان عشرًا لم يعد قرارًا واحدًا يسع كل المسابقات — فصار سياسةً تُعلن وتُجمَّد
   * وتُدقَّق كبقية السياسات.
   */

  /**
   * متى يُسند المتسابق إلى لجنته.
   * - `ON_ARRIVAL`: لحظة مسح البطاقة (الافتراضي) — يناسب الحضور المتدفّق.
   * - `PRE_ASSIGNED`: خطةٌ تُعتمد قبل اليوم — يناسب الوفود التي تحتاج معرفةً مسبقة.
   * - `WAVES`: موجاتٌ تُوزَّع دفعةً واحدة — يناسب اللجان الكثيرة، لأن الدفعة تسمح
   *   بالتبديل قبل أن يصير الإسناد قرارًا يمسّ الأسبقية.
   * - `BY_CATEGORY`: اللجنة تُختار بالفئة — يناسب القاعة التي تُعلن لجانها على الفئات،
   *   فتُقدَّم اللجنة المكرَّسة لفئة المتسابق على المشتركة، ولا يُسنَد خارج فئته أبدًا.
   */
  distributionMode?: 'ON_ARRIVAL' | 'PRE_ASSIGNED' | 'WAVES' | 'BY_CATEGORY';

  /**
   * حين لا تؤهّل المتسابقَ أيُّ لجنة.
   * - `INCIDENT` (الافتراضي): لا إسناد، وتُفتح حادثة توجيه — العطب يُرى عند البوابة
   *   حيث يمكن إصلاحه، لا عند بدء الجلسة والمتسابق جالس.
   * - `ANY_AVAILABLE`: السلوك القديم — أي لجنة متاحة ولو لم تحكم فئته.
   */
  unmatchedArrivalPolicy?: 'INCIDENT' | 'ANY_AVAILABLE';

  /** هل تُشترط لجنةٌ مؤهَّلة لرواية المتسابق عند الإسناد؟ */
  requireReadingQualifiedPanel?: boolean;

  /**
   * أقصى حصّةٍ لوفدٍ واحد من طابور لجنة (٠–١). يمنع وقوع وفدٍ كامل تحت لجنةٍ واحدة —
   * وهو ما يجعل تفاوت اللجان الذي يقيسه `committee-integrity` أثرًا على وفدٍ بعينه.
   * قيدٌ تفضيليّ: يُخفَّف صراحةً ويُسجَّل حين لا يبقى بديلٌ مؤهَّل.
   */
  delegationShareCap?: number;

  /** أقصى عدد منتظرين في طابور لجنة واحدة. صفرٌ أو غياب = بلا سقف. */
 ...
 * أثرُ الموافقة.
 *
 * و`version` وحدها لا تكفي منذ صارت الوثائق على ثلاث طبقات (الجهة ← المشغّل ← المنصّة):
 * جهتان كلتاهما «١.٠» لا تُفرَّقان، ولا يُعرف على وثيقة مَن وقّع الموقِّع. فيُكتب معها
 * **الناشر** وطبقتُه ورابطُ الوثيقة وتاريخ سريانها — وهي ما يُحتجّ به يوم النزاع.
 * وتبقى اختيارية لأن أثرًا قديمًا كُتب قبلها لا يُعاد كتابته بأثرٍ رجعيّ.
 */
export interface ConsentRecord { id:string; participantId:string; competitionId:string; kind:'terms'|'privacy'|'audio_recording'|'ai_processing'|'guardian'|'human_review'|'ai_inference'|'ai_validation'|'ai_training'|'scientific_research'|'external_dataset_publication'; version:string; accepted:boolean; acceptedAt:string; guardianName?:string; publisher?:string; publisherLevel?:'organization'|'operator'|'platform'; documentUrl?:string; documentEffectiveDate?:string; }
